...
def league_ensure_npc_scores(week_start):
    """اگه هنوز برای این هفته امتیاز تیم‌های رقیب ساخته نشده، الان می‌سازه (فقط بار اول)."""
    with db_lock:
        conn = get_conn()
        ...
        for npc_key, (_, power) in LEAGUE_NPC_TEAMS.items():
            score = max(0, round(power * random.uniform(0.3, 1.3)))
            try:
                conn.execute(
                    "INSERT INTO league_npc_scores (week_start, npc_key, score) VALUES (?, ?, ?)",
                    (week_start, npc_key, score)
                )
            except Exception:
                pass
        conn.commit()
