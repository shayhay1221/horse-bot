...
def handle_digit_reply(message):
    ...
    if gamble_custom and gamble_custom["message_id"] == reply_id:
        ...
        user_id = message.from_user.id
        if game == "slot":
            ok2, text = start_slot_spin(chat_id, user_id, bet)
...
GAMBLE_MIN_BET = 20
GAMBLE_MAX_BET = 1000
GAMBLE_PRESET_BETS = [20, 50, 100, 250, 500]
...
def start_slot_spin(chat_id, user_id, bet):
    """خروجی: (ok, text). جای بازی رو قبل از کم‌کردن پول رزرو می‌کنیم که
    دو تا کلیک هم‌زمان هر دو نتونن از چک رد بشن و باعث گم‌شدن یه شرط نشن."""
    with slot_lock:
        if (chat_id, user_id) in _slot_spin_pending:
            return False, "یه اسلات برات باز مونده، اول اونو بچرخون."
    ...
    if not taken:
        ...
        return False, f"شیهه‌پوینت کافی نداری! تو فقط {remaining} تا داری."
...
def start_coin_streak(chat_id, user_id, side, bet):
    with streak_lock:
        if (chat_id, user_id) in _streak_pending:
            return False, "یه بازی برات باز مونده، اول اونو تموم کن.", None
...
def start_mines(chat_id, user_id, bet, num_mines):
    with mines_lock:
        if (chat_id, user_id) in _mines_games:
            return False, "یه بازی مین‌یاب برات باز مونده، اول اونو تموم کن.", None
...
def handle_gamble_callback(call):
    ...
    if action.startswith("bet"):
        ...
        if game == "slot":
            bet = int(parts[2])
            ok, text = start_slot_spin(chat_id, user_id, bet)
            if not ok:
