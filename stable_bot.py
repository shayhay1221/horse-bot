...
def poker_kb_lobby(chat_id, can_start):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(
        f"🪑 ورود به میز (ورودی: {POKER_ANTE} شیهه‌پوینت)", callback_data=f"pk:join:{chat_id}"
    ))
    if can_start:
...
def poker_status_text(game: PokerGame, extra_note: str = ""):
    ...
    lines = [
        f"🃏 پوکر (تگزاس هولدم) — pot: {game.pot} شیهه‌پوینت",
    ...
    if owe > 0:
        lines.append(f"\n🎯 نوبت: {turn_name} — باید {owe} شیهه‌پوینت بیاره (Call) یا Raise بزنه.")
    else:
...
def poker_handle_join(call, chat_id):
    ...
    if not u or (u.get("shihe_points") or 0) < POKER_ANTE:
        bot.answer_callback_query(
            call.id, f"برای نشستن سر میز حداقل {POKER_ANTE} شیهه‌پوینت لازم داری.", show_alert=True
        )
        return
    ...
    bot.edit_message_text(
        f"🃏 میز پوکر (تگزاس هولدم، ورودی هر نفر {POKER_ANTE} شیهه‌پوینت)\n"
        f"میز ({len(game.players)}/{POKER_MAX_PLAYERS}): {names}",
        chat_id, call.message.message_id,
        reply_markup=poker_kb_lobby(chat_id, can_start=can_start and not full) if not full else None,
    )
...
def poker_start_hand(chat_id):
    ...
    for uid in game.players:
        ...
        if not u or (u.get("shihe_points") or 0) < POKER_ANTE:
            bot.send_message(
                chat_id,
                f"{game.player_names.get(uid, uid)} دیگه {POKER_ANTE} شیهه‌پوینت نداره، بازی لغو شد."
            )
            delete_poker_game(chat_id)
            return
    ...
    game.pot = POKER_ANTE * len(game.players)
    for uid in game.players:
    ...
    bot.send_message(
        chat_id,
        f"🃏 دست شروع شد! وجه رو میز: {game.pot} شیهه‌پوینت\n"
...
def poker_handle_call(call, chat_id):
    ...
    if not u or (u.get("shihe_points") or 0) < owe:
        # شیهه‌پوینت کافی نیست — خودکار فولد می‌شه
        game.folded.append(user_id)
        save_poker_game(game)
        bot.answer_callback_query(call.id, "شیهه‌پوینت کافی نداری، فولد شدی.", show_alert=True)
        poker_after_fold(chat_id, game, user_id)
        return

    adjust_shihe(user_id, -owe)
    game.pot += owe
...
def poker_handle_raise_reply(message, game, user_id, amount):
    ...
    u = get_user_row(user_id)
    if not u or (u.get("shihe_points") or 0) < need:
        bot.reply_to(message, f"شیهه‌پوینت کافی نداری. برای این رایز {need} تا لازمه.")
        return

    adjust_shihe(user_id, -need)
    game.pot += need
...
def poker_payout_single_winner(chat_id, game, winner_id):
    adjust_shihe(winner_id, game.pot)
    bot.send_message(
        chat_id,
        f"🏆 همه فولد کردن، {game.player_names.get(winner_id, winner_id)} "
        f"بدون رو کردن کارت‌ها {game.pot} شیهه‌پوینت رو برد!",
    )
    delete_poker_game(chat_id)
...
def poker_showdown(chat_id):
    ...
    if len(winners) == 1:
        ...
        lines.append(f"\n🏆 برنده: {win_name} — {game.pot} شیهه‌پوینت برد!")
    else:
...
def handle_poker_raise_text(message):
    chat_id = message.chat.id
    game = load_poker_game(chat_id)
    if not game or game.phase != "betting" or not game.game_message_id:
        return
    if message.reply_to_message.message_id != game.game_message_id:
        return
...
def bj_kb_lobby(chat_id, can_start):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(
        f"🪑 ورود به میز (شرط: {BJ_BET} شیهه‌پوینت)", callback_data=f"bj:join:{chat_id}"
    ))
    if can_start:
...
def bj_status_text(game: BlackjackGame, extra_note: str = ""):
    lines = [f"🂡 بلک‌جک — شرط هر نفر: {BJ_BET} شیهه‌پوینت\n"]
    ...
    for uid in game.players:
        ...
        if uid in game.natural:
            ...
            tag = " ✋ ایستاد"
        lines.append(f"{game.get_name(uid)}: {' '.join(str(c) for c in hand)} = {val}{tag}")
...
def cmd_blackjack(message):
    ...
    bot.send_message(
        chat_id,
        f"🂡 میز بلک‌جک باز شد! (شرط هر نفر: {BJ_BET} شیهه‌پوینت)\n"
...
def bj_handle_join(call, chat_id):
    ...
    ensure_user(user.id, user.username or user.first_name or "")
    u = get_user_row(user.id)
    if not u or (u.get("shihe_points") or 0) < BJ_BET:
        bot.answer_callback_query(call.id, f"برای نشستن سر میز حداقل {BJ_BET} شیهه‌پوینت لازم داری.", show_alert=True)
        return
    ...
    bot.edit_message_text(
        f"🂡 میز بلک‌جک (شرط هر نفر: {BJ_BET} شیهه‌پوینت)\n"
        f"میز ({len(game.players)}/{BJ_MAX_PLAYERS}): {names}",
        chat_id, call.message.message_id,
        reply_markup=bj_kb_lobby(chat_id, can_start=can_start and not full) if not full else None,
    )
...
def bj_start_hand(chat_id):
    ...
    for uid in game.players:
        ...
        if not u or (u.get("shihe_points") or 0) < BJ_BET:
            bot.send_message(
                chat_id,
                f"{game.player_names.get(uid, uid)} دیگه {BJ_BET} شیهه‌پوینت نداره، بازی لغو شد."
            )
            delete_blackjack_game(chat_id)
            return
...
def bj_dealer_play(chat_id, note=""):
    ...
    for uid in game.players:
        ...
        if uid in game.busted:
            lines.append(f"{name}: {value} — ❌ باخت، {BJ_BET} شیهه‌پوینت از دست رفت.")
            continue
        ...
        if uid in game.natural:
            if dealer_value == 21 and len(dealer_hand) == 2:
                ...
                win = int(BJ_BET * 2.5)
                adjust_shihe(uid, win)
                lines.append(f"{name}: بلک‌جک طبیعی! 🎉 — {win} شیهه‌پوینت برد.")
            continue

        if dealer_busted or value > dealer_value:
            adjust_shihe(uid, BJ_BET * 2)
            lines.append(f"{name}: {value} — ✅ برد، {BJ_BET * 2} شیهه‌پوینت گرفت.")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print("۱۵ ثانیه صبر می‌کنیم و دوباره تلاش می‌کنیم...")
            time.sleep(15)
