...
def hokm_start_new_hand(chat_id):
    ...
    bot.send_message(
        ...
        f"حاکم این دست: {game.player_names[hakem_id]}",
    )
    ...
    if hakem_pm_ok:
        bot.send_message(
            chat_id,
            f"🔔 {game.player_names[hakem_id]} حاکمه — باید تو پیوی خال حکم رو انتخاب کنه:",
            reply_markup=hokm_kb_open_pm(),
        )
    else:
        bot.send_message(
...
def hokm_update_live_board(game):
    """پیام لایو‌بورد تو گروه رو می‌سازه یا ادیت می‌کنه تا همه بدون رفتن به پیوی ببینن دست چطوره."""
    text = hokm_board_text(game)
    if game.trick_msg_id:
        try:
            bot.edit_message_text(text, game.chat_id, game.trick_msg_id)
            return
        except Exception:
            pass
    try:
        sent = bot.send_message(game.chat_id, text)
...
def hokm_send_turn_prompt(game):
    ...
    if pm_ok:
        bot.send_message(
            game.chat_id,
            f"🔔 نوبت {game.player_names[uid]}ه — کارتات تو پیویته:",
            reply_markup=hokm_kb_open_pm(),
        )
    else:
        bot.send_message(
