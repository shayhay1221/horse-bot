...
def handle_digit_reply(message):
    ...
    if gamble_custom and gamble_custom["message_id"] == reply_id:
        ...
        user_id = message.from_user.id
        if game == "slot":
            _, text = resolve_slot(user_id, bet)
            bot.reply_to(message, text)
        elif game == "coin":
            _, text = resolve_coin(user_id, extra, bet)
            bot.reply_to(message, text)
        elif game == "dice":
            _, text = resolve_dice(user_id, extra, bet)
            bot.reply_to(message, text)
        elif game == "hilo":
...
# و مبلغ دلخواه هم از طریق همون handle_digit_reply مرکزیِ موجود
# گرفته می‌شه (نه یه message_handler جدا) — که با توجه به رفتار
# first-match-breaks کتابخونه، این تنها راه امنشه.

GAMBLE_MIN_BET = 20
GAMBLE_MAX_BET = 1000
GAMBLE_PRESET_BETS = [20, 50, 100, 250, 500]

_gamble_pending = {}          # key: (chat_id, user_id) -> {"bet": int, "card": int}  (دست باز بیشتر/کمتر)
...
mines_lock = threading.Lock()  # جلوگیری از double-payout اگه دو تا کلیک (مثلاً دابل‌تپ رو «برداشت») هم‌زمان برسن
...
def gamble_kb_higher_lower(chat_id, user_id):
    kb = types.InlineKeyboardMarkup()
    kb.row(
        types.InlineKeyboardButton("🔼 بیشتر", callback_data=f"gamble:hl:higher:{chat_id}:{user_id}"),
        types.InlineKeyboardButton("🔽 کمتر", callback_data=f"gamble:hl:lower:{chat_id}:{user_id}"),
    )
    return kb
...
SLOT_TRIPLE_MULTIPLIER = {"7️⃣": 20, "💎": 15}
SLOT_TRIPLE_DEFAULT_MULTIPLIER = 8
SLOT_PAIR_MULTIPLIER = 2


def resolve_slot(user_id, bet):
    """خروجی: (ok, text). ok=False یعنی فقط موجودی کافی نبود و شرطی گرفته نشد."""
    ...
    if reels[0] == reels[1] == reels[2]:
        ...
        text = f"🎰 {reel_text}\n🎉 سه‌تایی شد! بردی: {winnings} شیهه‌پوینت (x{multiplier})\n🌟 موجودی: {new_balance}"
    elif reels[0] == reels[1] or reels[1] == reels[2] or reels[0] == reels[2]:
        winnings = bet * SLOT_PAIR_MULTIPLIER
        _, new_balance = adjust_shihe(user_id, winnings)
        text = f"🎰 {reel_text}\n✨ دوتایی! بردی: {winnings} شیهه‌پوینت (x{SLOT_PAIR_MULTIPLIER})\n🌟 موجودی: {new_balance}"
    else:
...
def resolve_coin(user_id, side, bet):
    taken, remaining = adjust_shihe(user_id, -bet)
    if not taken:
        return False, f"شیهه‌پوینت کافی نداری! تو فقط {remaining} تا داری."
    ...
    if result == side:
        winnings = int(bet * COINFLIP_WIN_MULTIPLIER)
        _, new_balance = adjust_shihe(user_id, winnings)
        text = f"🪙 سکه: {result}!\n🎉 درست حدس زدی! بردی: {winnings} شیهه‌پوینت\n🌟 موجودی: {new_balance}"
    else:
        text = f"🪙 سکه: {result}!\n😢 باختی. {bet} شیهه‌پوینت رفت.\n🌟 موجودی: {remaining}"
...
def start_hilo(chat_id, user_id, bet):
    """خروجی: (ok, text, keyboard_or_None)"""
    if (chat_id, user_id) in _gamble_pending:
        return False, "یه دست بیشتر/کمتر برات باز مونده، اول اونو تموم کن.", None
    ...
    _gamble_pending[(chat_id, user_id)] = {"bet": bet, "card": card}
    text = f"🃏 کارت اول: {card}\nحدس بزن کارت بعدی بیشتره یا کمتره:"
    return True, text, gamble_kb_higher_lower(chat_id, user_id)
...
def resolve_dice(user_id, dice_type, bet):
    """dice_type: 'even' | 'odd' | 'exactN' (مثلاً exact7)"""
    taken, remaining = adjust_shihe(user_id, -bet)
    if not taken:
        return False, f"شیهه‌پوینت کافی نداری! تو فقط {remaining} تا داری."
    ...
    total = d1 + d2
    dice_line = f"🎲 {d1} + {d2} = {total}"

    if dice_type == "even":
        won, multiplier, label = total % 2 == 0, DICE_EVEN_ODD_MULTIPLIER, "زوج"
    elif dice_type == "odd":
        won, multiplier, label = total % 2 == 1, DICE_EVEN_ODD_MULTIPLIER, "فرد"
    else:
        target = int(dice_type.replace("exact", ""))
        won, multiplier, label = total == target, DICE_EXACT_MULTIPLIERS.get(target, 8), f"عدد {target}"

    if won:
        winnings = int(bet * multiplier)
        _, new_balance = adjust_shihe(user_id, winnings)
        text = f"{dice_line}\n🎉 شرطت ({label}) درست بود! بردی: {winnings} شیهه‌پوینت (x{multiplier})\n🌟 موجودی: {new_balance}"
    else:
...
def mines_kb(chat_id, user_id):
    ...
    kb = types.InlineKeyboardMarkup(row_width=MINES_TOTAL_CELLS)
    row = []
...
def mines_final_kb(state, hit_idx=None):
    """صفحه‌ی نهایی: همه‌ی مین‌ها و خونه‌های باز‌شده رو نشون می‌ده، دکمه‌ها غیرفعالن."""
    kb = types.InlineKeyboardMarkup(row_width=MINES_TOTAL_CELLS)
    row = []
    for idx in range(MINES_TOTAL_CELLS):
        if idx == hit_idx:
...
def start_mines(chat_id, user_id, bet, num_mines):
    ...
    with mines_lock:
        ...
        # جا رو رزرو می‌کنیم قبل از کم کردن شیهه‌پوینت، که دو تا کلیک هم‌زمان
        # هر دو نتونن از این چک رد بشن و باعث دو بار کم شدن/گم شدن شرط نشن.
...
def handle_gamble_callback(call):
    ...
    if action.startswith("bet"):
        ...
        if game == "coin":
            side = parts[2]
            bet = int(parts[3])
            ok, text = resolve_coin(user_id, side, bet)
            keyboard = None
        elif game == "dice":
            dice_type = parts[2]
            bet = int(parts[3])
            ok, text = resolve_dice(user_id, dice_type, bet)
            keyboard = None
        elif game == "mines":
            ...
            bet = int(parts[2])
            ok, text = resolve_slot(user_id, bet)
            keyboard = None
        else:  # hilo
            bet = int(parts[2])
    ...
    if action == "hl":
        guess = parts[2]
        owner_chat_id = int(parts[3])
        owner_user_id = int(parts[4])

        if call.from_user.id != owner_user_id:
            bot.answer_callback_query(call.id, "این شرط مال تو نیست!", show_alert=True)
            return

        pending = _gamble_pending.pop((owner_chat_id, owner_user_id), None)
        if pending is None:
            bot.answer_callback_query(call.id, "این شرط منقضی شده یا قبلاً جواب داده شده.", show_alert=True)
            return
        ...
        if second_card == first_card:
            _, new_balance = adjust_shihe(owner_user_id, bet)  # مساوی: شرط برمی‌گرده
            result_text = f"🃏 کارت اول: {first_card} | کارت دوم: {second_card}\n🤝 مساوی شد، شرطت بدون کم و کسر برگشت.\n🌟 موجودی: {new_balance}"
        else:
            won = (guess == "higher" and second_card > first_card) or (guess == "lower" and second_card < first_card)
            if won:
                winnings = int(bet * HILO_WIN_MULTIPLIER)
                _, new_balance = adjust_shihe(owner_user_id, winnings)
                result_text = f"🃏 کارت اول: {first_card} | کارت دوم: {second_card}\n🎉 درست گفتی! بردی: {winnings} شیهه‌پوینت\n🌟 موجودی: {new_balance}"
            else:
                new_balance = (get_user_row(owner_user_id) or {}).get("shihe_points", 0)
                result_text = f"🃏 کارت اول: {first_card} | کارت دوم: {second_card}\n😢 باختی. {bet} شیهه‌پوینت رفت.\n🌟 موجودی: {new_balance}"

        bot.answer_callback_query(call.id)
        bot.edit_message_text(result_text, call.message.chat.id, call.message.message_id, reply_markup=_gamble_back_kb())
        return
