...
def create_dino_catch(user_id):
    role = random.choice(["thief", "guard"])
    today = _today_str(iran_now())
    with db_lock:
        conn = get_conn()
        conn.execute(
            "INSERT INTO dino_catches (user_id, role, caught_date) VALUES (?, ?, ?)",
            (user_id, role, today)
        )
        conn.commit()
        conn.close()
...
def get_guards_today():
    today = _today_str(iran_now())
    with db_lock:
        conn = get_conn()
        rows = conn.execute(
            "SELECT DISTINCT user_id FROM dino_catches WHERE role='guard' AND caught_date=?",
            (today,)
        ).fetchall()
        conn.close()
        return [r["user_id"] for r in rows]
...
def get_thieves_today():
    today = _today_str(iran_now())
    with db_lock:
        conn = get_conn()
        rows = conn.execute(
            "SELECT DISTINCT user_id FROM dino_catches WHERE role='thief' AND caught_date=?",
            (today,)
        ).fetchall()
        conn.close()
        return [r["user_id"] for r in rows]
...
def get_our_door_count(round_key):
    """محاسبه‌ی خام (بدون قفل) تعداد درها بر اساس تعداد نگهبانانی که همین الان ثبت‌نام کردن.
    توجه: این تابع مستقیم نباید جای دیگه‌ای صدا زده بشه چون هر بار نتیجه‌ش می‌تونه فرق کنه؛
    به‌جاش از get_locked_our_door_count استفاده کن که همون مقدار اولیه رو برای کل دور ثابت نگه می‌داره."""
    with db_lock:
        conn = get_conn()
        row = conn.execute(
            "SELECT COUNT(*) as cnt FROM dino_war_guards WHERE round_key=?", (round_key,)
        ).fetchone()
        conn.close()
        registered_guards = row["cnt"] if row else 0
    return max(2, registered_guards * 2)


def get_locked_our_door_count(round_key):
    """تعداد درهای شهر رو برای یه دور مشخص فقط یه‌بار (اولین باری که لازم میشه) محاسبه
    و ذخیره می‌کنه؛ دفعات بعد همون عدد ذخیره‌شده برگردونده میشه، حتی اگه نگهبان‌های
    بیشتری بعداً ثبت‌نام کنن. این‌جوری کسی که اول ثبت‌نام کرده وسط کار منوی متفاوتی نمی‌بینه."""
    with db_lock:
        conn = get_conn()
        row = conn.execute(
            "SELECT door_count FROM dino_war_door_counts WHERE round_key=?", (round_key,)
        ).fetchone()
        if row:
            conn.close()
    ...
    # هنوز قفل نشده -> بر اساس نگهبان‌های ثبت‌نام‌شده‌ی الان محاسبه و ذخیره کن
    door_count = get_our_door_count(round_key)
    with db_lock:
        conn = get_conn()
        try:
            conn.execute(
                "INSERT INTO dino_war_door_counts (round_key, door_count) VALUES (?, ?)",
                (round_key, door_count)
            )
            conn.commit()
        except Exception:
            # یعنی هم‌زمان یکی دیگه هم داشت قفلش می‌کرد؛ مقدار ذخیره‌شده رو بخون
...
def register_guard_intent(round_key, user_id):
    """یه سهمیه‌ی نگهبانی جدید برای این کاربر تو این دور باز می‌کنه (در هنوز انتخاب نشده).
    برمی‌گردونه: id همین ردیف، که برای انتخاب در بهش نیاز داریم (چون الان یه نفر می‌تونه
    چند سهمیه/چند در همزمان داشته باشه، پس باید بدونیم کدوم سهمیه رو داریم آپدیت می‌کنیم)."""
    with db_lock:
        conn = get_conn()
        cur = conn.execute(
...
def resolve_war_round(round_key):
    ...
    for target, atk_list in thieves_per_target.items():
        ...
        guarded_door = random.randint(1, NPC_TARGET_DOOR_COUNT)  # نگهبانی NPC کاملاً شانسیه
        for atk in atk_list:
            any_activity = True
...
def send_hunt_message():
    ...
    text = (
        ...
        "روی همین پیام ریپلای بزن و 🎯 بفرست —\n"
        "اگه بولزای بزنی، یکیشونو رام می‌کنی!\n"
        "(هر نفر حداکثر ۳ تیر داره)"
    )
...
def handle_dart_reply(message):
    ...
    slot_key = current_hunt_slot_key()
    if not is_hunt_message(slot_key, message.chat.id, message.reply_to_message.message_id):
        return  # این ریپلای به گله‌ی امروز نیست (پیام قدیمیه)
    ...
    is_bullseye = (message.dice.value == 6)
    record_hunt_attempt(slot_key, user_id, caught_now=is_bullseye)

    if is_bullseye:
        role = create_dino_catch(user_id)
        role_display = "🦹 دزد" if role == "thief" else "🛡️ نگهبان"
        bot.reply_to(
            message,
            f"🎯 بولزای! یه دایناسور رام کردی!\nنقشش: {role_display}\n\n"
            f"تو جنگای امروز می‌تونه شرکت کنه."
        )
    else:
...
def send_war_announcement():
    round_key = current_war_round_key()
    with db_lock:
        conn = get_conn()
        row = conn.execute("SELECT announced FROM dino_war_rounds WHERE round_key=?", (round_key,)).fetchone()
        if row and row["announced"]:
            conn.close()
            return
        try:
            conn.execute(
                "INSERT INTO dino_war_rounds (round_key, announced) VALUES (?, 1) "
                "ON CONFLICT(round_key) DO UPDATE SET announced=1",
                (round_key,)
            )
        except Exception:
            pass
    ...
    guards_today = get_guards_today()
    thieves_today = get_thieves_today()

    for chat_id in _get_active_chats():
        try:
            if guards_today:
                kb = types.InlineKeyboardMarkup()
                kb.add(types.InlineKeyboardButton("✅ من هستم (نگهبانی)", callback_data=f"dino:guard_in:{round_key}"))
                bot.send_message(chat_id, "⚔️ جنگ شروع شد! کیا امروز می‌خوان تو دفاع شهر شرکت کنن؟\n(فقط کسایی که امروز دایناسور نگهبان گرفتن)", reply_markup=kb)
            if thieves_today:
                kb2 = types.InlineKeyboardMarkup()
                kb2.add(types.InlineKeyboardButton("🦹 حمله با دایناسور دزد", callback_data=f"dino:thief_in:{round_key}"))
                bot.send_message(chat_id, "🦹 کیا امروز می‌خوان حمله کنن؟\n(فقط کسایی که امروز دایناسور دزد گرفتن)", reply_markup=kb2)
        except Exception:
            pass
...
def force_send_war_announcement():
    """مثل send_war_announcement ولی به فلگ «قبلاً اعلام شده» کاری نداره، همیشه می‌فرسته.
    فقط برای تست دستیه؛ ریزالو شدن دور جنگ (نتیجه‌گیری نهایی) طبق زمان‌بندی خودش، بدون تغییر، انجام میشه."""
    round_key = current_war_round_key()

    # فلگ رو هم آپدیت می‌کنیم که سیستم خودکار دوباره همون رو نفرسته
    with db_lock:
        conn = get_conn()
        try:
            conn.execute(
    ...
    for chat_id in _get_active_chats():
        try:
            if guards_today:
                kb = types.InlineKeyboardMarkup()
                kb.add(types.InlineKeyboardButton("✅ من هستم (نگهبانی)", callback_data=f"dino:guard_in:{round_key}"))
                bot.send_message(chat_id, "⚔️ (تست دستی) جنگ شروع شد! کیا امروز می‌خوان تو دفاع شهر شرکت کنن؟\n(فقط کسایی که امروز دایناسور نگهبان گرفتن)", reply_markup=kb)
            if thieves_today:
                kb2 = types.InlineKeyboardMarkup()
                kb2.add(types.InlineKeyboardButton("🦹 حمله با دایناسور دزد", callback_data=f"dino:thief_in:{round_key}"))
                bot.send_message(chat_id, "🦹 (تست دستی) کیا امروز می‌خوان حمله کنن؟\n(فقط کسایی که امروز دایناسور دزد گرفتن)", reply_markup=kb2)
            if not guards_today and not thieves_today:
                bot.send_message(chat_id, "⚔️ (تست دستی) دور جنگ شروع شد، ولی امروز هنوز هیچ‌کس دایناسور نگهبان یا دزد نگرفته.")
        except Exception:
            pass
...
def render_guard_door_menu(round_key, slot_id):
    taken = get_taken_doors(round_key)
    door_count = get_locked_our_door_count(round_key)
    kb = types.InlineKeyboardMarkup(row_width=4)
    buttons = []
    for i in range(1, door_count + 1):
        if i not in taken:
            buttons.append(types.InlineKeyboardButton(f"🚪 {i}", callback_data=f"dino:guard_door:{round_key}:{slot_id}:{i}"))
    kb.add(*buttons)
    return "🛡️ کدوم در می‌خوای وایستی؟\n(هر در فقط یه نگهبان می‌تونه داشته باشه)", kb
...
def is_round_still_open(round_key):
    """True فقط اگه round_key هنوز همون دور جنگ جاریه. اگه زمان از این دور گذشته
    و وارد بازه‌ی یه دور جدید شدیم، این False برمی‌گردونه (یعنی دکمه‌های اون دور دیگه منقضی شدن)."""
    return round_key == current_war_round_key()
...
def handle_dino_callback(call):
    ...
    if action == "guard_in":
        round_key = parts[2]
        catch_count = get_user_role_catch_count(user_id, "guard")
        if catch_count == 0:
            bot.answer_callback_query(call.id, "امروز دایناسور نگهبان نداری!", show_alert=True)
            return
        slots_used = get_user_guard_slots_used(round_key, user_id)
        if slots_used >= catch_count:
            bot.answer_callback_query(call.id, f"سهمیه‌ی نگهبانیت برای این دور تموم شده ({catch_count} دایناسور نگهبان داشتی، هر کدوم یه در).", show_alert=True)
            return
        slot_id = register_guard_intent(round_key, user_id)
        text, kb = render_guard_door_menu(round_key, slot_id)
        bot.send_message(call.message.chat.id, text, reply_markup=kb)
        bot.answer_callback_query(call.id)

    elif action == "guard_door":
        round_key, slot_id_str, door_str = parts[2], parts[3], parts[4]
        ...
        round_key = parts[2]
        catch_count = get_user_role_catch_count(user_id, "thief")
        if catch_count == 0:
            bot.answer_callback_query(call.id, "امروز دایناسور دزد نداری!", show_alert=True)
            return
        attacks_used = get_user_thief_attacks_used(round_key, user_id)
        if attacks_used >= catch_count:
            bot.answer_callback_query(call.id, f"سهمیه‌ی دزدیت برای این دور تموم شده ({catch_count} دایناسور دزد داشتی، هر کدوم یه حمله).", show_alert=True)
            return
        ...
        remaining = catch_count - (attacks_used + 1)
        extra = f"\n(هنوز {remaining} تا دایناسور دزدِ دیگه داری که می‌تونی باهاش به یه هدف دیگه حمله کنی، از همون دکمه‌ی «حمله با دایناسور دزد» دوباره بزن.)" if remaining > 0 else ""
        bot.edit_message_text(
    ...
    while True:
        ...
        try:
            ...
            if current.hour in WAR_HOURS and current.minute < 1:
                round_key = current_war_round_key(current)
                if _get_flag(f"dino_war_announced_{round_key}") != "1":
                    send_war_announcement()
                    _set_flag(f"dino_war_announced_{round_key}", "1")

            # رزولوشن جنگ (نیم‌ساعت قبل از جنگ بعدی)
            round_key = current_war_round_key(current)
            next_key = next_war_round_key(round_key)
            next_dt = datetime.fromisoformat(next_key)
            resolve_time = next_dt - timedelta(minutes=30)
