...
import crime_gang  # سیستم خلافکاری و گنگ — فایل جداست، فقط register() می‌شه
import restaurant_job  # شغل رستوران — فایل جداست، فقط register() می‌شه

# برای پخش کارت (حکم) و چیدمان کشتی‌ها (نبرد دریایی) از تصادفی‌سازی سطح
# سیستم‌عامل استفاده می‌کنیم (همون چیزی که برای رمزنگاری به کار می‌ره)، نه
# فقط random معمولی — که کاملاً غیرقابل پیش‌بینی و عادلانه باشه.
_secure_rng = random.SystemRandom()

# =========================================================
#   موتورهای بازی (حکم و نبرد دریایی) — قبلاً فایل جدا بودن،
#   حالا همه‌چی تو همین یه فایله که مدیریتش راحت‌تر باشه.
# =========================================================

# ---------- موتور بازی حکم (از hokm_engine.py) ----------
...
class HokmGame:
    chat_id: int
    num_players: int = 4                                   # ۲ یا ۴
    players: list[int] = field(default_factory=list)
    player_names: dict[int, str] = field(default_factory=dict)

    phase: Phase = Phase.WAITING_PLAYERS
    dealer_index: int = 0
    hakem_index: Optional[int] = None
    ...
    tricks_won: dict[int, int] = field(default_factory=lambda: {0: 0, 1: 0})
    hands_won: dict[int, int] = field(default_factory=lambda: {0: 0, 1: 0})
    target_hands: int = 7
    last_hand_kout: bool = False
    trick_msg_id: Optional[int] = None  # پیام «لایو بورد» تو گروه که هر کارت رو ادیت می‌شه

    pending_deck: list[Card] = field(default_factory=list)  # بین شروع دست و انتخاب حکم
    round_history: list[dict] = field(default_factory=list)

    # -----------------------------------------------------------------
    # پارامترهای وابسته به تعداد نفرات
    # -----------------------------------------------------------------

    @property
    def full_hand_size(self) -> int:
        """۵۲ کارت تقسیم بر تعداد نفرات — ۱۳ برای ۴نفره، ۲۶ برای ۲نفره."""
        return 52 // self.num_players

    @property
    def total_tricks(self) -> int:
        return self.full_hand_size

    @property
    def win_threshold(self) -> int:
        """چند ترick لازمه تا یه تیم/نفر همون دست رو ببره (اکثریت)."""
        return self.total_tricks // 2 + 1
    ...
    def add_player(self, user_id: int, name: str) -> None:
        ...
        if len(self.players) >= self.num_players:
            raise HokmError(f"میز پره ({self.num_players} نفر کامله).")
        self.players.append(user_id)
        self.player_names[user_id] = name

    def team_of(self, user_id: int) -> int:
        """
        ۴نفره: صندلی ۰و۲ = تیم ۰، صندلی ۱و۳ = تیم ۱ (نفرات روبه‌رو هم‌تیمی).
        ۲نفره: هرکی مال خودشه (idx 0 یا 1 مستقیماً "تیم" خودشه).
        """
    ...
    def start_hand(self) -> None:
        if len(self.players) != self.num_players:
            raise HokmError(f"برای شروع بازی دقیقاً {self.num_players} نفر لازمه.")

        deck = make_deck()
        _secure_rng.shuffle(deck)

        self.hakem_index = (self.dealer_index + 1) % self.num_players
        hakem_id = self.players[self.hakem_index]

        self.hands = {p: [] for p in self.players}
        self.hands[hakem_id] = deck[:5]
        self.pending_deck = deck[5:]
    ...
    def choose_hokm(self, user_id: int, suit: str) -> None:
        ...
        # مرحله ۲: پخش بقیه‌ی کارت‌ها — حاکم به‌اندازه‌ی full_hand_size-5 اضافه
        # می‌گیره (چون ۵ تا داره)، بقیه هرکدوم full_hand_size تا.
        deck = self.pending_deck
        i = 0
        for offset in range(self.num_players):
            idx = (self.hakem_index + offset) % self.num_players
            pid = self.players[idx]
            n = (self.full_hand_size - 5) if idx == self.hakem_index else self.full_hand_size
    ...
    def play_card(self, user_id: int, card: Card) -> Optional[dict]:
        ...
        if (self.tricks_won[winner_team] >= self.win_threshold
                or sum(self.tricks_won.values()) == self.total_tricks):
            final_winner = winner_team if self.tricks_won[winner_team] >= self.win_threshold else self._majority_team()
            self._end_hand(final_winner)
    ...
    def _end_hand(self, winning_team: int) -> None:
        ...
        is_kout = self.tricks_won[losing_team] == 0
        self.hands_won[winning_team] += 2 if is_kout else 1
        self.last_hand_kout = is_kout
    ...
    # -----------------------------------------------------------------
    # سریالایز برای ذخیره توی دیتابیس (پرسیستنس بین ری‌استارت‌های Render)
    # -----------------------------------------------------------------
    ...
    def to_dict(self) -> dict:
        ...
        d["pending_deck"] = [c.to_str() for c in self.pending_deck]
        d.pop("round_history", None)
        return d

    @staticmethod
    def from_dict(d: dict) -> "HokmGame":
        game = HokmGame(chat_id=d["chat_id"], num_players=d.get("num_players", 4))
        ...
        game.pending_deck = [Card.from_str(s) for s in d.get("pending_deck", [])]
        ...
        game.last_hand_kout = d.get("last_hand_kout", False)
        game.trick_msg_id = d.get("trick_msg_id")
        return game
...
# ---------- موتور بازی نبرد دریایی (از battleship_engine.py) ----------

BOARD_SIZE = 7
SHIP_SIZES = [4, 3, 3, 2]


class BSPhase(str, Enum):
    WAITING_PLAYERS = "waiting_players"
    PLAYING = "playing"
    GAME_OVER = "game_over"
...
def _key_to_cell(k: str) -> tuple[int, int]:
    r, c = k.split(",")
    return int(r), int(c)


def _place_ships_randomly() -> list[list[tuple[int, int]]]:
    """چیدمان رندوم کشتی‌ها؛ کشتی‌ها نباید همدیگه رو قطع کنن (لمس کردن مجازه)."""
    for _attempt in range(500):
        ...
        for size in SHIP_SIZES:
            ...
            for _try in range(200):
                horizontal = _secure_rng.choice([True, False])
                if horizontal:
                    r = _secure_rng.randint(0, BOARD_SIZE - 1)
                    c = _secure_rng.randint(0, BOARD_SIZE - size)
                    cells = [(r, c + i) for i in range(size)]
                else:
                    r = _secure_rng.randint(0, BOARD_SIZE - size)
                    c = _secure_rng.randint(0, BOARD_SIZE - 1)
        ...
        if ok:
            return ships
    raise BattleshipError("چیدمان کشتی‌ها ممکن نشد — دوباره امتحان کن.")
...
class BattleshipGame:
    chat_id: int
    table_id: int = 0
    players: list[int] = field(default_factory=list)          # دقیقاً ۲ نفر
    player_names: dict[int, str] = field(default_factory=dict)

    phase: BSPhase = BSPhase.WAITING_PLAYERS
    turn_index: int = 0

    ships: dict[int, list[Ship]] = field(default_factory=dict)          # user_id -> کشتی‌های خودش
    shots: dict[int, dict[str, str]] = field(default_factory=dict)      # user_id -> {"r,c": "hit"/"miss"}
    ...
    def add_player(self, user_id: int, name: str) -> None:
        if self.phase != BSPhase.WAITING_PLAYERS:
            raise BattleshipError("بازی از قبل شروع شده.")
        if user_id in self.players:
            raise BattleshipError("قبلاً وارد این میز شدی.")
        if len(self.players) >= 2:
            raise BattleshipError("این میز پره (۲ نفر کامله).")
        self.players.append(user_id)
        self.player_names[user_id] = name

    def start_game(self) -> None:
        if len(self.players) != 2:
            raise BattleshipError("برای شروع دقیقاً ۲ نفر لازمه.")
        for pid in self.players:
            ship_cells = _place_ships_randomly()
            self.ships[pid] = [Ship(cells=cells) for cells in ship_cells]
            self.shots[pid] = {}
        self.turn_index = 0
        self.phase = BSPhase.PLAYING

    def opponent_of(self, user_id: int) -> int:
        return self.players[1] if self.players[0] == user_id else self.players[0]

    def fire(self, shooter_id: int, r: int, c: int) -> dict:
        if self.phase != BSPhase.PLAYING:
            raise BattleshipError("الان وقت بازی کردن نیست.")
        if self.players[self.turn_index] != shooter_id:
            raise BattleshipError("نوبت تو نیست.")
        if not (0 <= r < BOARD_SIZE and 0 <= c < BOARD_SIZE):
            raise BattleshipError("خونه‌ی نامعتبره.")

        key = _cell_key(r, c)
        if key in self.shots[shooter_id]:
            raise BattleshipError("قبلاً همین خونه رو زدی.")

        opponent_id = self.opponent_of(shooter_id)
        hit_ship: Optional[Ship] = None
        for ship in self.ships[opponent_id]:
        ...
        self.shots[shooter_id][key] = "hit" if is_hit else "miss"

        sunk = is_hit and hit_ship.is_sunk
        all_sunk = all(ship.is_sunk for ship in self.ships[opponent_id])
        ...
        result = {
            ...
            "hit": is_hit,
            "sunk": sunk,
            "sunk_ship_size": len(hit_ship.cells) if sunk else None,
            "game_over": all_sunk,
        }

        if all_sunk:
            self.phase = BSPhase.GAME_OVER
    ...
    # -----------------------------------------------------------------
    # سریالایز برای ذخیره توی دیتابیس
    # -----------------------------------------------------------------
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "ships": {
                pid: [{"cells": s.cells, "hits": list(s.hits)} for s in ships]
                for pid, ships in self.ships.items()
            },
            "shots": self.shots,
    ...
    @staticmethod
    def from_dict(d: dict) -> "BattleshipGame":
        game = BattleshipGame(chat_id=d["chat_id"], table_id=d.get("table_id", 0))
        game.players = d["players"]
        game.player_names = {int(k): v for k, v in d["player_names"].items()}
        game.phase = BSPhase(d["phase"])
        game.turn_index = d["turn_index"]
        game.ships = {
            int(pid): [
                Ship(cells=[tuple(cell) for cell in s["cells"]], hits=set(tuple(h) for h in s["hits"]))
                for s in ships
            ]
            for pid, ships in d["ships"].items()
        }
        game.shots = {int(pid): dict(shots) for pid, shots in d["shots"].items()}
        game.winner_id = d.get("winner_id")
        return game
...
BS_BOARD_SIZE = BOARD_SIZE  # سازگاری با اسمی که تو بخش تلگرام حکم استفاده شده
...
if DATABASE_URL:
    ...
    _pg_last_health_check = [0.0]  # لیست برای mutable closure؛ زمان آخرین تست سلامت اتصال
    ...
    def _get_persistent_pg_connection():
            ...
            try:
                if _pg_conn.closed:
                    ...
                    if _pg_conn.get_transaction_status() != psycopg2.extensions.TRANSACTION_STATUS_IDLE:
                        _pg_conn.rollback()
                    # تست سلامت اتصال (SELECT 1) رو فقط هر ۶۰ ثانیه یه‌بار انجام
                    # می‌دیم، نه هر بار. این چک یه رفت‌وبرگشت شبکه‌ی اضافه به
                    # سوپابیسه؛ اگه رو هر کوئری انجامش بدیم ربات کند می‌شه.
                    # همچنان جلوی قطعی بی‌کاری رو می‌گیره چون بعد از هر بی‌کاری
                    # طولانی (که خطرش هست) این ۶۰ ثانیه گذشته.
                    t = time.monotonic()
                    if t - _pg_last_health_check[0] > 60:
                        cur = _pg_conn.cursor()
                        cur.execute("SELECT 1")
                        cur.close()
                        _pg_last_health_check[0] = t
            except Exception:
                try:
                    _pg_conn.close()
                except Exception:
        ...
        if _pg_conn is None:
            _pg_conn = psycopg2.connect(
                DATABASE_URL,
                connect_timeout=10,
                # اگه یه کوئری بیش از ۱۵ ثانیه طول بکشه (مثلاً شبکه به سوپابیس
                # گیر کرده)، پستگرس خودش با خطا لغوش می‌کنه به‌جای این‌که برای
                # همیشه معلق بمونه و db_lock رو (که کل ربات بهش وابسته‌ست) قفل نگه داره.
                options="-c statement_timeout=15000",
            )
            _pg_conn.autocommit = False
            _pg_last_health_check[0] = time.monotonic()
        return _pg_conn
    ...
    class PGConnWrapper:
        ...
        def execute(self, query, params=()):
            ...
            try:
                ...
                # اتصال واقعاً مرده (نه فقط یه خطای کوئری معمولی). به جای تلاش
                # برای rollback روی یه اتصال مرده (که خودش هم ارور می‌ده)،
                # فقط ریست می‌کنیم تا دفعه‌ی بعدی get_conn() یه اتصال تازه بسازه.
                try:
                    self._conn.close()
                except Exception:
                    pass
...
def ensure_user(user_id, username):
    ...
    این تابع رو تلگرام تقریباً رو هر پیام/دکمه صدا می‌زنه، برای همینه که سعی می‌کنیم
    برای کاربرای قدیمی فقط با یه SELECT رد بشیم و UPDATE رو فقط وقتی واقعاً یوزرنیم
    عوض شده بزنیم (نه هر بار)."""
    with db_lock:
        conn = get_conn()
...
class _SilentExceptionHandler(telebot.ExceptionHandler):
    """
    جلوگیری از کرش کل ربات به خاطر یه ارور تو یه هندلر.
    telebot به صورت پیش‌فرض اکسپشن‌های داخل هندلرها (پیام، کالبک و...) رو
    تو یه ترد جدا نگه می‌داره و بعد تو حلقه‌ی اصلی polling دوباره پرتاب
    می‌کنه؛ همین باعث می‌شد کل infinity_polling بترکه و ربات ۱۵ ثانیه‌ای
    ری‌استارت بشه (مثلاً وقتی یه callback query قدیمی/منقضی جواب داده می‌شد).
    با return True می‌گیم «خودم مدیریتش کردم»، پس دیگه بالا پرتاب نمی‌شه.
    """

    def handle(self, exception):
        try:
            print(f"⚠️ خطای هندل‌شده (نادیده گرفته شد تا ربات نخوابه): {exception}")
        except Exception:
            pass
        return True


# اگه تلگرام (مخصوصاً پشت پروکسی) جواب یه درخواست رو ندونه، این دو تا باعث
# می‌شن به‌جای گیرکردن بی‌نهایت، بعد از این مدت خطا بده — تا حلقه‌ی پایین
# (که همین الان هست) بتونه دوباره تلاش کنه. READ_TIMEOUT از long_polling_timeout
# بیشتره تا جای مانور شبکه بمونه.
telebot.apihelper.CONNECT_TIMEOUT = 15
telebot.apihelper.READ_TIMEOUT = 40

# num_threads بیشتر: رستوران دکمه‌ای حالا تماس‌های تلگرام رو زیاد می‌کنه (هر کلیک ۲ تماس)؛
# با پیش‌فرض ۲ ترد کارگر، همین چندتا کلیک همزمان کل صف پردازش رو قفل می‌کرد.
bot = telebot.TeleBot(TOKEN, parse_mode=None, num_threads=8, exception_handler=_SilentExceptionHandler())

# سیستم خلافکاری و گنگ — جدول‌هاشو می‌سازه و دستورات /ساختن_گنگ و /خلافکاری رو ثبت می‌کنه
crime_gang.register(bot, get_conn, db_lock, adjust_coins, PK_AUTOINCREMENT, ensure_user, find_user_by_username)

# شغل رستوران — جدول‌شو می‌سازه و دستورات /رستوران، /دستورپخت، /باز_رستوران و ... رو ثبت می‌کنه
restaurant_job.register(bot, get_conn, db_lock, adjust_coins, PK_AUTOINCREMENT, ensure_user)
...
try:
    # مطمئن می‌شیم هیچ webhook فعالی نیست (وگرنه با polling تداخل می‌کنه و
    # ارور 409 می‌ده). drop_pending_updates=True هم پیام‌های قدیمی/معلق از
    # قبل رو دور می‌ریزه تا هر instance جدید با یه صف تمیز شروع کنه.
    bot.remove_webhook()
    time.sleep(1)
except Exception as _e:
    print(f"⚠️ پاک کردن webhook با خطا مواجه شد (مشکلی نیست، ادامه می‌دیم): {_e}")
...
def handle_version(message):
    ...
    bot.reply_to(message, "🔖 نسخه: v-2026-09-01-add-blackjack-game")
...
# =========================================================
#              🔄 تبدیل شیهه‌پوینت به سکه 🔄
# =========================================================

SHIHE_TO_COIN_RATE = 1000  # هر ۱۰۰۰ شیهه‌پوینت = ۱ سکه


@bot.message_handler(commands=["تبدیل_شیهه"])
@group_only
def handle_convert_shihe(message):
    user_id = message.from_user.id
    parts = message.text.strip().split()
    if len(parts) < 2 or not parts[1].isdigit():
        bot.reply_to(
            message,
            f"طرز استفاده: /تبدیل_شیهه مقدار\n"
            f"نرخ تبدیل: هر {SHIHE_TO_COIN_RATE} شیهه‌پوینت = ۱ سکه\n"
            f"مقدار باید مضربی از {SHIHE_TO_COIN_RATE} باشه.\n"
            f"مثال: /تبدیل_شیهه {SHIHE_TO_COIN_RATE * 5}",
        )
        return
    ...
    if amount % SHIHE_TO_COIN_RATE != 0:
        bot.reply_to(
            message,
            f"مقدار باید مضربی از {SHIHE_TO_COIN_RATE} باشه (وگرنه باقی‌مونده هدر میره).\n"
            f"نزدیک‌ترین مضرب‌ها: {(amount // SHIHE_TO_COIN_RATE) * SHIHE_TO_COIN_RATE} یا "
            f"{((amount // SHIHE_TO_COIN_RATE) + 1) * SHIHE_TO_COIN_RATE}",
        )
        return

    coins_earned = amount // SHIHE_TO_COIN_RATE
    ...
    bot.reply_to(
        message,
        f"✅ {amount} شیهه‌پوینت تبدیل شد به {coins_earned} سکه!\n"
        f"شیهه‌پوینت باقی‌مونده: {remaining}",
    )
...
/تبدیل_شیهه مقدار — تبدیل شیهه‌پوینت به سکه (هر ۱۰۰۰ تا = ۱ سکه)

━━━ 🌟 شیهه‌پوینت ━━━
بنویس «شیهه» (بدون /) هر ۵ دقیقه یه‌بار برای گرفتن پوینت
...
/بازی — منوی بازی‌های گروهی (حکم، نبرد دریایی، پوکر)
/پوکر — پوکر تگزاس هولدم (۲ تا ۶ نفره، با شیهه‌پوینت)
/بلک_جک — بلک‌جک در برابر دیلر (تا ۶ نفره، با شیهه‌پوینت)

━━━ 🎰 گمبل (با شیهه‌پوینت) ━━━
/گمبل — منوی شرط‌بندی: اسلات، شیر یا خط، بیشتر یا کمتر، تاس، مین‌یاب
/لغو_مین — لغو مین‌یابِ بازِ فعلیت (با تأیید)

━━━ 🏴 خلافکاری و گنگ ━━━
/ساختن_گنگ — راه انداختن گنگ خودت
/خلافکاری — منوی گنگ: نیروها، استخدام، حقوق، روابط
/گنگ_رابطه <اسم گنگ> — وضعیت رابطه با یه گنگ خاص
/اتحاد <یوزرنیم> — پیشنهاد اتحاد برای یه سرقت گروهی
...
def handle_start(message):
    if not is_group(message):
        # اینجا دقیقاً همون‌جاییه که deep link حکم (t.me/ربات?start=hokm) میاد.
        # مهم نیست پیام چی میگه — همین که یه پیام رد و بدل شد، تلگرام از این به
        # بعد اجازه‌ی فرستادن پیام خصوصی به این کاربر رو به ربات میده.
        parts = message.text.split(maxsplit=1)
        payload = parts[1] if len(parts) > 1 else ""
        if payload == "hokm":
            bot.reply_to(message, "✅ آماده شدی! حالا برگرد تو گروه و «ورود به میز» رو دوباره بزن 🃏")
        else:
            bot.reply_to(message, "سلام! این ربات فقط توی گروه بازی می‌کنه — منو تو یه گروه اضافه کن یا از یه گروه که توشم استفاده کن 🐴")
        return
    mark_chat_active(message.chat.id)
    user_id = message.from_user.id
    username = message.from_user.username or message.from_user.first_name or ""
...
def build_main_menu(user_id=None):
    ...
    kb.add(
        types.InlineKeyboardButton("🏴 خلافکاری", callback_data="menu:crime"),
        types.InlineKeyboardButton("🍽️ رستوران", callback_data="menu:restaurant"),
    )
    kb.add(
        types.InlineKeyboardButton("🎮 بازی‌های گروهی", callback_data="menu:games"),
    )
    kb.add(
        types.InlineKeyboardButton("🎰 گمبل", callback_data="menu:gamble_open"),
    )
    kb.add(
...
def handle_callback(call):
    ...
    if data == "menu:crime":
        crime_gang.open_menu_from_main(call, user_id, call.message.chat.id, call.message.message_id)
        return
    ...
    if data.startswith("hokm:"):
        handle_hokm_callback(call)
        return

    if data.startswith("bs:"):
        handle_battleship_callback(call)
        return

    if data.startswith("pk:"):
        handle_poker_callback(call)
        return

    if data.startswith("bj:"):
        handle_blackjack_callback(call)
        return

    if data.startswith("game:"):
        handle_game_menu_callback(call)
        return

    if data.startswith("gamble:"):
        handle_gamble_callback(call)
        return
    ...
    try:
        if data == "menu:main":
            ...
            text, kb = "🎮 چند نفره بازی می‌کنین؟", game_kb_playercount()

        elif data == "menu:gamble_open":
            text, kb = "🎰 گمبل — کدوم بازی؟", gamble_kb_main_menu()
...
def iran_now():
    # datetime.utcnow() تو نسخه‌های جدید پایتون deprecated شده؛ این معادل
    # درستش با روش جدیده و همون خروجی naive (بدون tzinfo) رو می‌ده تا با
    # بقیه‌ی جاهای کد که از now() ساده استفاده می‌کنن سازگار بمونه.
    return datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(hours=IRAN_UTC_OFFSET_HOURS)
...
def resolve_war_round(round_key):
    ...
    try:
        _round_dt = _round_key_to_dt(round_key)
        _round_label = _round_dt.strftime("%H:%M")
    except Exception:
        _round_label = round_key
    lines = [f"⚔️ نتیجه‌ی جنگ ساعت {_round_label}:\n"]
    any_activity = False
...
def _new_war_round_key():
    ...
    اعلام). عمداً بدون کاراکتر ':' ساخته می‌شه -- چون callback_data دکمه‌ها با ':'
    از هم جدا می‌شن (data.split(":"))، و round_key به فرمت قبلی (مثل
    "2026-08-23T12:00:00") خودش شامل ':' بود؛ یعنی وقتی رو دکمه کلیک می‌شد، خودِ
    round_key موقع پارس شدن به چند تیکه می‌شکست و با چیزی که تو دیتابیس ذخیره شده
    بود یکی نمی‌شد -> همیشه «پیدا نشد» و در نتیجه پیام «این دور به پایان رسیده»."""
    return iran_now().strftime("%Y%m%dT%H%M%S%f")


def _round_key_to_dt(round_key):
    return datetime.strptime(round_key, "%Y%m%dT%H%M%S%f")
...
def is_round_still_open(round_key):
    ...
    with db_lock:
        ...
        conn.close()
    if row is None:
        print(f"[DINO-DEBUG] is_round_still_open({round_key!r}) -> False چون هیچ ردیفی با این round_key تو dino_war_rounds پیدا نشد")
        return False
    if row["resolved"]:
        print(f"[DINO-DEBUG] is_round_still_open({round_key!r}) -> False چون resolved=1 هست تو دیتابیس (announced_at={row['announced_at']!r})")
        return False
    if row["announced_at"]:
        announced_at = _round_key_to_dt(row["announced_at"])
        deadline = announced_at + timedelta(hours=WAR_ROUND_DEADLINE_HOURS)
        now = iran_now()
        if now >= deadline:
            print(f"[DINO-DEBUG] is_round_still_open({round_key!r}) -> False چون زمان گذشته: الان={now.isoformat()!r} >= مهلت={deadline.isoformat()!r} (announced_at={announced_at.isoformat()!r})")
            return False
    else:
        print(f"[DINO-DEBUG] is_round_still_open({round_key!r}) -> announced_at خالیه/NULL تو دیتابیس! چک مهلت رد شد (این خودش یعنی ستون درست ست نشده)")
    return True
...
def handle_dino_callback(call):
    ...
    if action == "guard_in":
        ...
        def _resolve_hunter_shot(round_key=round_key, hunt_id=hunt_id, chosen_dir=chosen_dir):
            # این تابع با تأخیر و تو یه ترد جدا اجرا می‌شه (به‌جای time.sleep
            # مستقیم تو هندلر) تا وقتی منتظر نمایش نتیجه‌ی شلیک هستیم، تردِ
            # پردازش کالبک‌های تلگرام رو برای این چند ثانیه اشغال نکنیم.
            try:
                hunt = get_hunt_by_id(hunt_id)
                ...
                print(f"⚠️ خطا تو resolve شلیک شکارچی: {_e}")

        threading.Timer(HUNTER_SHOT_DELAY_SECONDS, _resolve_hunter_shot).start()
...
def _dino_dart_handler(message):
    if message.dice and message.dice.emoji == "🎯":
        ...
        handle_slot_spin_reply(message)
    ...
    while True:
        ...
        try:
            ...
            for r in row:
                ...
                announced_at = _round_key_to_dt(r["announced_at"])
...
def run_dummy_web_server():
    ...
    # از ThreadingHTTPServer به‌جای HTTPServer استفاده می‌کنیم تا اگه چند تا
    # درخواست (مثلاً از UptimeRobot) همزمان بیان، هر کدوم تو یه ترد جدا
    # پردازش بشن و یکی گیر کردن باعث صف شدن بقیه نشه.
...
#                    🃏  بازی حکم  🃏
# =========================================================
# منطق خالص بازی (قوانین، نوبت‌بندی، امتیازدهی) تو hokm_engine.py هست —
# این فایل باید کنار stable_bot.py روی همون ریپو باشه.
#
# کارت‌ها تو پیوی هرکس فرستاده می‌شه (رازداری کامل، دقیقاً مثل حکم واقعی).
# برای کم کردن اصطکاک «باید اول پیوی رو باز کنی»، از deep link تلگرام
# استفاده می‌کنیم: یه دکمه که با یه لمس هم پیوی رو باز می‌کنه هم /start
# رو خودکار می‌زنه. اگه بازیکنی وسط بازی پیوی رو بست/بلاک کرد، همون‌جا تو
# گروه بهش خبر می‌دیم که دوباره باز کنه، بازی معطل می‌مونه تا برگرده (پیام
# قبلی که ارسال نشد رو نمی‌فرستیم مجدد، ولی نوبتش سر جاشه).
...
def hokm_pm_deep_link():
    username = get_bot_username()
    if not username:
        return None
    return f"https://t.me/{username}?start=hokm"


def hokm_can_pm(user_id):
    """چک می‌کنه ربات می‌تونه به این کاربر پیام خصوصی بده یا نه."""
    try:
...
def hokm_kb_open_pm():
    link = hokm_pm_deep_link()
    kb = types.InlineKeyboardMarkup()
    if link:
        kb.add(types.InlineKeyboardButton("🔓 باز کردن پیوی با ربات", url=link))
    return kb


def init_hokm_tables():
    with db_lock:
        conn = get_conn()
        c = conn.cursor()
        c.execute("""
...
def save_hokm_game(game: HokmGame):
    state_json = json.dumps(game.to_dict(), ensure_ascii=False)
    with db_lock:
        conn = get_conn()
        conn.execute(
            "INSERT INTO hokm_games (chat_id, state) VALUES (?, ?) "
...
def load_hokm_game(chat_id):
    with db_lock:
        conn = get_conn()
        row = conn.execute("SELECT state FROM hokm_games WHERE chat_id=?", (chat_id,)).fetchone()
        conn.close()
    if not row:
        return None
    return HokmGame.from_dict(json.loads(row["state"]))


def delete_hokm_game(chat_id):
    with db_lock:
        conn = get_conn()
        conn.execute("DELETE FROM hokm_games WHERE chat_id=?", (chat_id,))
        conn.commit()
        conn.close()
...
def hokm_kb_mode():
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        types.InlineKeyboardButton("👥 ۲ نفره", callback_data="hokm:mode:2"),
        types.InlineKeyboardButton("👨‍👩‍👧‍👦 ۴ نفره", callback_data="hokm:mode:4"),
    )
    return kb
...
def hokm_kb_join():
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🪑 ورود به میز", callback_data="hokm:join"))
    return kb


def hokm_kb_suits(chat_id):
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        types.InlineKeyboardButton(f"♠ {SUIT_NAMES_FA['♠']}", callback_data=f"hokm:suit:{chat_id}:♠"),
        types.InlineKeyboardButton(f"♥ {SUIT_NAMES_FA['♥']}", callback_data=f"hokm:suit:{chat_id}:♥"),
    )
    kb.add(
        types.InlineKeyboardButton(f"♦ {SUIT_NAMES_FA['♦']}", callback_data=f"hokm:suit:{chat_id}:♦"),
...
def hokm_kb_cards(chat_id, cards):
    kb = types.InlineKeyboardMarkup(row_width=4)
    buttons = [
        types.InlineKeyboardButton(str(c), callback_data=f"hokm:play:{chat_id}:{c.to_str()}")
        for c in cards
    ]
...
def hokm_format_hand(cards):
    return " ".join(str(c) for c in cards)


# ---------- دستورها ----------

@bot.message_handler(commands=["حکم"])
@group_only
def cmd_hokm(message):
    chat_id = message.chat.id
    existing = load_hokm_game(chat_id)
    if existing and existing.phase != Phase.MATCH_OVER:
        bot.reply_to(message, "یه بازی حکم همین الان تو این گروه در جریانه. اگه گیر کرده، /لغو_حکم رو بزن.")
        return

    bot.send_message(chat_id, "🃏 بازی حکم! چند نفره بازی می‌کنین؟", reply_markup=hokm_kb_mode())


@bot.message_handler(commands=["لغو_حکم"])
@group_only
def cmd_cancel_hokm(message):
    chat_id = message.chat.id
    if load_hokm_game(chat_id):
        delete_hokm_game(chat_id)
        bot.reply_to(message, "بازی حکم لغو شد.")
    else:
        bot.reply_to(message, "بازی فعالی برای لغو کردن نیست.")


# ---------- هندلر کال‌بک ----------

def handle_hokm_callback(call):
    data = call.data
    parts = data.split(":")
    action = parts[1]

    if action == "mode":
        hokm_handle_mode(call, int(parts[2]))
    elif action == "join":
        hokm_handle_join(call)
    elif action == "suit":
        hokm_handle_choose_suit(call, int(parts[2]), parts[3])
    elif action == "play":
        hokm_handle_play_card(call, int(parts[2]), parts[3])
    else:
        bot.answer_callback_query(call.id)


def hokm_handle_mode(call, num_players):
    chat_id = call.message.chat.id
    existing = load_hokm_game(chat_id)
    if existing and existing.phase != Phase.MATCH_OVER:
        bot.answer_callback_query(call.id, "یه بازی حکم دیگه همین الان در جریانه.", show_alert=True)
        return

    game = HokmGame(chat_id=chat_id, num_players=num_players)
    save_hokm_game(game)
    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        f"🃏 بازی حکم {num_players}نفره شروع شد!\n"
        f"منتظر بقیه‌ایم — هرکی می‌خواد بازی کنه دکمه رو بزنه:\n\nمیز: (۰/{num_players})",
        chat_id, call.message.message_id,
        reply_markup=hokm_kb_join(),
    )


def hokm_handle_join(call):
    chat_id = call.message.chat.id
    user = call.from_user

    game = load_hokm_game(chat_id)
    if not game or game.phase != Phase.WAITING_PLAYERS:
        bot.answer_callback_query(call.id, "این بازی دیگه فعال نیست.", show_alert=True)
        return

    if not hokm_can_pm(user.id):
        bot.answer_callback_query(
            call.id,
            "اول باید پیوی ربات رو باز کنی (پایین گروه یه دکمه فرستادم).",
            show_alert=True,
        )
        bot.send_message(
            chat_id,
            f"{user.first_name} عزیز، چون کارتاتو باید خصوصی بفرستم، اول این دکمه رو بزن "
            f"(خودکار پیوی رو باز و /start می‌زنه)، بعد برگرد و «ورود به میز» رو دوباره بزن:",
            reply_markup=hokm_kb_open_pm(),
        )
        return
    ...
    save_hokm_game(game)
    bot.answer_callback_query(call.id, f"وارد میز شدی، {user.first_name}!")

    names = " · ".join(game.player_names[p] for p in game.players)
    bot.edit_message_text(
        f"🃏 بازی حکم {game.num_players}نفره\nمنتظر بقیه‌ایم...\n\n"
        f"میز ({len(game.players)}/{game.num_players}): {names}",
        chat_id, call.message.message_id,
        reply_markup=hokm_kb_join() if len(game.players) < game.num_players else None,
    )

    if len(game.players) == game.num_players:
        hokm_start_new_hand(chat_id)


def hokm_start_new_hand(chat_id):
    game = load_hokm_game(chat_id)
    game.start_hand()
    save_hokm_game(game)

    if game.num_players == 4:
        team1 = f"{game.player_names[game.players[0]]} و {game.player_names[game.players[2]]}"
        team2 = f"{game.player_names[game.players[1]]} و {game.player_names[game.players[3]]}"
        teams_line = f"🔵 تیم ۱: {team1}\n🔴 تیم ۲: {team2}\n\n"
    else:
        teams_line = (
            f"🔵 {game.player_names[game.players[0]]}  در مقابل  "
            f"🔴 {game.player_names[game.players[1]]}\n\n"
        )

    hakem_id = game.players[game.hakem_index]

    bot.send_message(
        chat_id,
        f"{teams_line}"
        f"دیلر: {game.player_names[game.players[game.dealer_index]]}\n"
        f"حاکم این دست: {game.player_names[hakem_id]} — باید تو پیوی خال حکم رو انتخاب کنه:",
        reply_markup=hokm_kb_open_pm(),
    )
    ...
    try:
        bot.send_message(
            hakem_id,
            f"حاکمی! این ۵ کارتته:\n{hokm_format_hand(game.hands[hakem_id])}\n\nخال حکم رو انتخاب کن:",
            reply_markup=hokm_kb_suits(chat_id),
        )
    except Exception:
        bot.send_message(
            chat_id,
            f"⚠️ نتونستم به {game.player_names[hakem_id]} پیام خصوصی بدم. باید پیویش رو باز کنه:",
            reply_markup=hokm_kb_open_pm(),
        )


def hokm_handle_choose_suit(call, chat_id, suit):
    user = call.from_user
    game = load_hokm_game(chat_id)
    if not game or game.phase != Phase.CHOOSING_HOKM:
        bot.answer_callback_query(call.id, "الان وقت انتخاب حکم نیست.", show_alert=True)
        return
    if game.players[game.hakem_index] != user.id:
        bot.answer_callback_query(call.id, "فقط حاکم می‌تونه خال حکم رو انتخاب کنه.", show_alert=True)
        return

    try:
        game.choose_hokm(user.id, suit)
    except HokmError as e:
        bot.answer_callback_query(call.id, str(e), show_alert=True)
        return
    ...
    save_hokm_game(game)
    bot.answer_callback_query(call.id)
    bot.edit_message_text(f"🃏 حکم شد: {SUIT_NAMES_FA[suit]} {suit}", call.message.chat.id, call.message.message_id)

    bot.send_message(chat_id, f"🃏 حکم این دست: {SUIT_NAMES_FA[suit]} {suit}\nکارت‌ها الان تو پیوی هرکس می‌ره...")

    for pid in game.players:
        try:
            bot.send_message(pid, f"دست تو این دوره:\n{hokm_format_hand(game.hands[pid])}")
        except Exception:
            bot.send_message(
                chat_id,
                f"⚠️ نتونستم به {game.player_names[pid]} پیام بدم — باید پیویش رو باز کنه:",
                reply_markup=hokm_kb_open_pm(),
            )

    hokm_send_turn_prompt(game)
    save_hokm_game(game)


def hokm_board_text(game):
    if game.current_trick:
        played = " | ".join(f"{game.player_names[p]}: {c}" for p, c in game.current_trick)
    else:
        played = "هنوز کسی نزده"
    hakem_name = game.player_names[game.players[game.hakem_index]]
    turn_name = game.player_names[game.players[game.turn_index]]
    return (
        f"🎯 حکم: {SUIT_NAMES_FA[game.hokm_suit]} {game.hokm_suit} (حاکم: {hakem_name})\n"
        f"🔵 {game.tricks_won[0]} – 🔴 {game.tricks_won[1]}\n\n"
        f"روی دست: {played}\n"
        f"نوبت: {turn_name}"
    )


def hokm_update_live_board(game):
    """پیام لایو‌بورد تو گروه رو می‌سازه یا ادیت می‌کنه؛ دکمه‌ی رفتن به پیوی هم روشه."""
    text = hokm_board_text(game)
    kb = hokm_kb_open_pm()
    if game.trick_msg_id:
        try:
            bot.edit_message_text(text, game.chat_id, game.trick_msg_id, reply_markup=kb)
            return
        except Exception:
            pass
    try:
        sent = bot.send_message(game.chat_id, text, reply_markup=kb)
        game.trick_msg_id = sent.message_id
    except Exception:
        pass


def hokm_send_turn_prompt(game):
    uid = game.players[game.turn_index]
    legal = game.legal_cards(uid)

    hokm_update_live_board(game)  # همینجا هم وضعیت رو نشون می‌ده هم دکمه‌ی پیوی رو می‌ذاره

    lead_info = ""
    if game.current_trick:
        lead_suit = game.current_trick[0][1].suit
        played_so_far = " ".join(f"{game.player_names[p]}:{c}" for p, c in game.current_trick)
        lead_info = f"روی دست: {played_so_far}\nباید {SUIT_NAMES_FA[lead_suit]} بزنی (اگه داری).\n\n"

    try:
        bot.send_message(
            uid,
            f"نوبت توئه!\n{lead_info}کارت‌هات:",
            reply_markup=hokm_kb_cards(game.chat_id, legal),
        )
    except Exception:
        bot.send_message(
            game.chat_id,
            f"⚠️ نوبت {game.player_names[uid]}ه ولی نمی‌تونم پیوی بدم — باید پیویش رو باز کنه:",
            reply_markup=hokm_kb_open_pm(),
        )


def hokm_handle_play_card(call, chat_id, card_str):
    user = call.from_user
    game = load_hokm_game(chat_id)
    if not game or game.phase != Phase.PLAYING:
        bot.answer_callback_query(call.id, "الان وقت بازی کردن نیست.", show_alert=True)
        return
    if game.players[game.turn_index] != user.id:
        bot.answer_callback_query(call.id, "نوبت تو نیست.", show_alert=True)
        return

    card = Card.from_str(card_str)
    try:
        result = game.play_card(user.id, card)
    except HokmError as e:
        bot.answer_callback_query(call.id, str(e), show_alert=True)
        return

    bot.answer_callback_query(call.id)
    bot.edit_message_text(f"زدی: {card} ✅", call.message.chat.id, call.message.message_id)

    if result:
        lines = " | ".join(f"{game.player_names[p]}: {c}" for p, c in result["trick"])
        team_label = _hokm_team_label(game, result["team"])
        bot.send_message(
            chat_id,
            f"{lines}\n🏆 برنده: {game.player_names[result['winner_id']]} ({team_label})\n"
            f"🔵 {result['tricks_won'][0]} – 🔴 {result['tricks_won'][1]}",
        )
        game.trick_msg_id = None  # ترick بعدی یه بورد تازه می‌گیره

    if game.phase == Phase.PLAYING:
        hokm_send_turn_prompt(game)
        save_hokm_game(game)
    elif game.phase == Phase.HAND_OVER:
        save_hokm_game(game)
        winner_team = 0 if game.tricks_won[0] > game.tricks_won[1] else 1
        team_label = _hokm_team_label(game, winner_team)
        kout_line = "😤 کُــت! حریف حتی یه دست هم نبرد — این دست ۲ امتیاز حساب شد.\n" if game.last_hand_kout else ""
        bot.send_message(
            chat_id,
            f"✅ دست تموم شد! {team_label} این دست رو برد ({game.tricks_won[0]} - {game.tricks_won[1]})\n"
            f"{kout_line}"
            f"امتیاز کلی مسابقه → 🔵 {game.hands_won[0]} - 🔴 {game.hands_won[1]}\n\n"
            f"دست بعدی داره شروع می‌شه...",
        )
        hokm_start_new_hand(chat_id)
    elif game.phase == Phase.MATCH_OVER:
        winner_team = 0 if game.hands_won[0] >= game.target_hands else 1
        team_label = _hokm_team_label(game, winner_team)
        bot.send_message(
            chat_id,
            f"🎉 مسابقه تموم شد! برنده: {team_label} با {game.hands_won[winner_team]} دست!\nدمتون گرم 👏",
        )
        delete_hokm_game(chat_id)


def _hokm_team_label(game, team_id):
    if game.num_players == 4:
        return "🔵 تیم ۱" if team_id == 0 else "🔴 تیم ۲"
    pid = game.players[team_id]
    return f"{'🔵' if team_id == 0 else '🔴'} {game.player_names[pid]}"
...
# =========================================================
#                 🚢  نبرد دریایی (Battleship)  🚢
# =========================================================
# منطق خالص بازی تو battleship_engine.py هست — این فایل هم باید کنار
# stable_bot.py روی همون ریپو باشه.
#
# برخلاف حکم، این بازی نیازی به پیوی نداره: تنها چیز مخفی (چیدمان
# کشتی‌ها) کاملاً رندومه و هیچ‌کس باهاش تعامل نمی‌کنه، پس امن می‌مونه.
#
# پشتیبانی از چند میز هم‌زمان تو یه گروه: هر میز یه table_id جدا داره
# (نه اینکه فقط یه بازی هم‌زمان مجاز باشه مثل حکم).

def init_battleship_tables():
    with db_lock:
        conn = get_conn()
        c = conn.cursor()
        c.execute(f"""
...
def create_battleship_table(chat_id) -> int:
    """یه ردیف جدید می‌سازه و table_id رو برمی‌گردونه (با RETURNING —
    هم رو Postgres هم رو SQLite مدرن کار می‌کنه، برخلاف lastrowid که رو
    Postgres کار نمی‌کنه)."""
    with db_lock:
        conn = get_conn()
        cur = conn.execute(
            "INSERT INTO battleship_games (chat_id, state) VALUES (?, ?) RETURNING id",
            (chat_id, "{}")
        )
        row = cur.fetchone()
        conn.commit()
...
def save_battleship_game(game: BattleshipGame):
    state_json = json.dumps(game.to_dict(), ensure_ascii=False)
    with db_lock:
        conn = get_conn()
        conn.execute(
            "UPDATE battleship_games SET state=? WHERE id=?",
            (state_json, game.table_id)
        )
        conn.commit()
        conn.close()


def load_battleship_game(table_id) -> "BattleshipGame | None":
    with db_lock:
        conn = get_conn()
        row = conn.execute("SELECT state FROM battleship_games WHERE id=?", (table_id,)).fetchone()
        conn.close()
    if not row or row["state"] == "{}":
        return None
    return BattleshipGame.from_dict(json.loads(row["state"]))


def delete_battleship_table(table_id):
    with db_lock:
        conn = get_conn()
        conn.execute("DELETE FROM battleship_games WHERE id=?", (table_id,))
        conn.commit()
        conn.close()
...
def bs_kb_lobby(table_id):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🪑 ورود به این میز", callback_data=f"bs:join:{table_id}"))
    kb.add(types.InlineKeyboardButton("❌ لغو میز", callback_data=f"bs:cancel:{table_id}"))
    return kb


def bs_kb_board(table_id, shots: dict):
    kb = types.InlineKeyboardMarkup(row_width=BS_BOARD_SIZE)
    for r in range(BS_BOARD_SIZE):
        row_buttons = []
        for c in range(BS_BOARD_SIZE):
            key = f"{r},{c}"
            status = shots.get(key)
            if status == "hit":
                label = "💥"
            elif status == "miss":
                label = "🌊"
                ...
                label = "・"
            row_buttons.append(
                types.InlineKeyboardButton(label, callback_data=f"bs:fire:{table_id}:{r}:{c}")
            )
        kb.add(*row_buttons)
    return kb
...
@bot.message_handler(commands=["نبرد_دریایی"])
@group_only
def cmd_battleship(message):
    chat_id = message.chat.id
    table_id = create_battleship_table(chat_id)
    game = BattleshipGame(chat_id=chat_id, table_id=table_id)
    save_battleship_game(game)

    bot.send_message(
        chat_id,
        f"🚢 میز نبرد دریایی #{table_id} باز شد!\nمنتظر نفر دومیم — هرکی می‌خواد بازی کنه دکمه رو بزنه:\n\nمیز: (۰/۲)",
        reply_markup=bs_kb_lobby(table_id),
    )


# ---------- هندلر کال‌بک ----------

def handle_battleship_callback(call):
    data = call.data
    parts = data.split(":")
    action = parts[1]

    if action == "join":
        bs_handle_join(call, int(parts[2]))
    elif action == "cancel":
        bs_handle_cancel(call, int(parts[2]))
    elif action == "fire":
        bs_handle_fire(call, int(parts[2]), int(parts[3]), int(parts[4]))
    else:
        bot.answer_callback_query(call.id)


def bs_handle_join(call, table_id):
    user = call.from_user
    game = load_battleship_game(table_id)
    if not game or game.phase != BSPhase.WAITING_PLAYERS:
        bot.answer_callback_query(call.id, "این میز دیگه فعال نیست.", show_alert=True)
        return
    ...
    if len(game.players) < 2:
        names = " · ".join(game.player_names[p] for p in game.players)
        bot.edit_message_text(
            f"🚢 میز نبرد دریایی #{table_id}\nمنتظر نفر دومیم...\n\nمیز (۱/۲): {names}",
            call.message.chat.id, call.message.message_id,
            reply_markup=bs_kb_lobby(table_id),
        )
        return

    # نفر دوم اومد -> بازی شروع می‌شه
    game.start_game()
    save_battleship_game(game)

    names = " ⚔️ ".join(game.player_names[p] for p in game.players)
    bot.edit_message_text(
        f"🚢 میز #{table_id} کامل شد!\n{names}\n\nکشتی‌ها چیده شدن، بازی شروع شد!",
        call.message.chat.id, call.message.message_id,
    )

    bs_send_turn_prompt(game)


def bs_handle_cancel(call, table_id):
    user = call.from_user
    game = load_battleship_game(table_id)
    if not game:
        bot.answer_callback_query(call.id, "این میز دیگه وجود نداره.", show_alert=True)
        return
    if game.players and user.id not in game.players:
        bot.answer_callback_query(call.id, "فقط کسی که تو میزه می‌تونه لغوش کنه.", show_alert=True)
        return

    delete_battleship_table(table_id)
    bot.answer_callback_query(call.id, "میز لغو شد.")
    bot.edit_message_text(
        f"🚢 میز نبرد دریایی #{table_id} لغو شد.",
        call.message.chat.id, call.message.message_id,
    )


def bs_send_turn_prompt(game: BattleshipGame):
    shooter_id = game.players[game.turn_index]
    opponent_id = game.opponent_of(shooter_id)
    bot.send_message(
        game.chat_id,
        f"🎯 نوبت {game.player_names[shooter_id]}ه — رو تخته‌ی {game.player_names[opponent_id]} بزن:",
        reply_markup=bs_kb_board(game.table_id, game.shots[shooter_id]),
    )


def bs_handle_fire(call, table_id, r, c):
    user = call.from_user
    game = load_battleship_game(table_id)
    if not game or game.phase != BSPhase.PLAYING:
        bot.answer_callback_query(call.id, "این بازی دیگه فعال نیست.", show_alert=True)
        return
    if game.players[game.turn_index] != user.id:
        bot.answer_callback_query(call.id, "نوبت تو نیست.", show_alert=True)
    ...
    if result["hit"]:
        bot.answer_callback_query(call.id, "زدی! 💥")
    else:
        bot.answer_callback_query(call.id, "آب بود 🌊")

    bot.edit_message_reply_markup(
        call.message.chat.id, call.message.message_id,
        reply_markup=bs_kb_board(table_id, game.shots[user.id]),
    )

    if result["sunk"]:
        bot.send_message(
            game.chat_id,
            f"💥🚢 {game.player_names[user.id]} یه کشتی {result['sunk_ship_size']}تایی از "
            f"{game.player_names[result['opponent_id']]} رو غرق کرد!",
        )

    if result["game_over"]:
        bot.send_message(
            game.chat_id,
            f"🏆 {game.player_names[user.id]} برد! همه‌ی کشتی‌های "
            f"{game.player_names[result['opponent_id']]} غرق شدن. 🎉",
        )
        delete_battleship_table(table_id)
    else:
        bs_send_turn_prompt(game)
...
# =========================================================
#                🃏  بازی پوکر (تگزاس هولدم ساده)  🃏
# =========================================================
# نسخه‌ی ساده‌شده‌ی تگزاس هولدم: هرکی ۲ کارت خصوصی می‌گیره، ۵ کارت
# مشترک (فلاپ/ترن/ریور) رو میز باز می‌شه، و بین هر مرحله یه دور
# شرط‌بندی داریم. دکمه‌های Check/Call رو خود پیام بازی تو گروهه؛
# برای Raise باید رو پیام بازی ریپلای کنی و مبلغ رو بنویسی.
# ورودی هر نفر و شرط‌ها با شیهه‌پوینت حساب می‌شه، نه سکه.
# کارت‌های دست (خصوصی) از طریق پیوی فرستاده می‌شه، دقیقاً مثل حکم.

from itertools import combinations

POKER_ANTE = 50          # ورودی هر نفر برای شروع دست (شیهه‌پوینت)
POKER_MIN_PLAYERS = 2
POKER_MAX_PLAYERS = 6

POKER_ROUND_NAMES_FA = {
    "preflop": "قبل از فلاپ",
    "flop": "فلاپ",
    "turn": "ترن",
    "river": "ریور",
...
def poker_evaluate_hand(cards: list[Card]) -> tuple:
    """امتیاز یه دست ۵ کارتی رو به‌صورت یه tuple قابل مقایسه برمی‌گردونه (بزرگ‌تر بهتره)."""
    ...
    if len(unique_ranks) == 5:
        ...
        elif unique_ranks == [RANK_VALUE["A"], RANK_VALUE["5"], RANK_VALUE["4"], RANK_VALUE["3"], RANK_VALUE["2"]]:
            # استریت ویژه: آس-۲-۳-۴-۵ (اینجا آس کمترین ارزش رو داره)
            is_straight = True
            straight_high = RANK_VALUE["5"]

    counts = Counter(ranks)
    ordered = sorted(counts.items(), key=lambda kv: (-kv[1], -kv[0]))
    count_values = [kv[1] for kv in ordered]
    ordered_ranks = [kv[0] for kv in ordered]
    ...
    if count_values[0] == 2 and count_values[1] == 2:
        return (2, *ordered_ranks[:3])
    if count_values[0] == 2:
        return (1, *ordered_ranks)
    return (0, *ranks)


def poker_best_of_seven(cards7: list[Card]) -> tuple:
    """از بین ۷ کارت (۲ دست + ۵ میز)، بهترین ترکیب ۵تایی رو پیدا می‌کنه."""
    return max(poker_evaluate_hand(list(combo)) for combo in combinations(cards7, 5))


def poker_hand_name(score: tuple) -> str:
    return POKER_HAND_NAMES_FA[score[0]]


@dataclass
class PokerGame:
    chat_id: int
    phase: str = "waiting"                                  # waiting, betting, done
    players: list[int] = field(default_factory=list)        # ترتیب ثابت همه‌ی بازیکن‌ها
    player_names: dict[int, str] = field(default_factory=dict)
    folded: list[int] = field(default_factory=list)
    hole_cards: dict[str, list[str]] = field(default_factory=dict)   # str(user_id) -> ["A♠","K♥"]
    community: list[str] = field(default_factory=list)
    deck: list[str] = field(default_factory=list)
    pot: int = 0
    betting_round: str = "preflop"                          # preflop, flop, turn, river
    current_bet: int = 0                                     # مجموع شرط این دور که باید بهش رسید
    round_contrib: dict[str, int] = field(default_factory=dict)      # str(user_id) -> مبلغ واریزی این دور
    acted: list[int] = field(default_factory=list)          # کی‌ها تو این دور یه اکشن زدن
    turn_index: int = 0                                      # ایندکس تو players
    game_message_id: Optional[int] = None                    # پیام اصلی بازی تو گروه (برای ریپلای رایز)
    ante: int = 50                                           # ورودی هر نفر (نفر اول موقع باز کردن میز تعیینش می‌کنه)
    practice: bool = False                                   # حالت تمرینی — بدون شیهه‌پوینت واقعی
    practice_balance: dict[str, int] = field(default_factory=dict)   # str(user_id) -> موجودی خیالی این میز
    ...
    def from_dict(d):
        game = PokerGame(**d)
        # کلیدهای دیکشنری بعد از json.loads همیشه رشته می‌شن؛ برای
        # player_names (که با کلید عددی نوشته می‌شه) باید برگردن به int،
        # وگرنه lookup با user_id عددی بعد از یه بار ذخیره/بارگذاری میشکنه.
        game.player_names = {int(k): v for k, v in game.player_names.items()}
        return game
    ...
    def get_name(self, user_id) -> str:
        return self.player_names.get(int(user_id), str(user_id))

    def get_hole(self, user_id) -> list[Card]:
        return [Card.from_str(s) for s in self.hole_cards[str(user_id)]]

    def get_community(self) -> list[Card]:
        return [Card.from_str(s) for s in self.community]

    def unit_name(self) -> str:
        return "امتیاز تمرینی" if self.practice else "شیهه‌پوینت"


POKER_PRACTICE_BANKROLL = 100  # موجودی خیالی هر نفر تو حالت تمرینی


def poker_get_balance(game: PokerGame, user_id) -> int:
    if game.practice:
        return game.practice_balance.get(str(user_id), 0)
    row = get_user_row(user_id)
    return (row.get("shihe_points") or 0) if row else 0


def poker_pay(game: PokerGame, user_id, delta: int) -> None:
    """delta مثبت = واریز به حساب بازیکن، منفی = کسر. تو حالت تمرینی فقط رو موجودی خیالی همین میز اثر می‌ذاره."""
    if game.practice:
        key = str(user_id)
        game.practice_balance[key] = game.practice_balance.get(key, 0) + delta
    else:
        adjust_shihe(user_id, delta)
...
def save_poker_game(game: PokerGame):
    state_json = json.dumps(game.to_dict(), ensure_ascii=False)
    with db_lock:
        conn = get_conn()
        conn.execute(
            "INSERT INTO poker_games (chat_id, state) VALUES (?, ?) "
            "ON CONFLICT(chat_id) DO UPDATE SET state=excluded.state",
            (game.chat_id, state_json)
        )
        conn.commit()
...
def load_poker_game(chat_id):
    with db_lock:
        conn = get_conn()
        row = conn.execute("SELECT state FROM poker_games WHERE chat_id=?", (chat_id,)).fetchone()
        conn.close()
    if not row:
        return None
    return PokerGame.from_dict(json.loads(row["state"]))


def delete_poker_game(chat_id):
    with db_lock:
        conn = get_conn()
        conn.execute("DELETE FROM poker_games WHERE chat_id=?", (chat_id,))
        conn.commit()
        conn.close()
...
def poker_kb_lobby(chat_id, can_start, ante=None, practice=False):
    label_amount = ante if ante is not None else POKER_ANTE
    unit = "امتیاز تمرینی" if practice else "شیهه‌پوینت"
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(
        f"🪑 ورود به میز (ورودی: {label_amount} {unit})", callback_data=f"pk:join:{chat_id}"
    ))
    if can_start:
        kb.add(types.InlineKeyboardButton("🎬 شروع دست", callback_data=f"pk:start:{chat_id}"))
    kb.add(types.InlineKeyboardButton("📖 راهنما", callback_data=f"pk:help:{chat_id}"))
    return kb


def poker_kb_turn(chat_id, owe):
    kb = types.InlineKeyboardMarkup(row_width=2)
    if owe > 0:
        kb.add(types.InlineKeyboardButton(f"✅ Call ({owe})", callback_data=f"pk:call:{chat_id}"))
    else:
        kb.add(types.InlineKeyboardButton("✋ Check", callback_data=f"pk:check:{chat_id}"))
    kb.add(types.InlineKeyboardButton("📈 Raise", callback_data=f"pk:raiseinfo:{chat_id}"))
    kb.add(types.InlineKeyboardButton("🏳️ فولد", callback_data=f"pk:fold:{chat_id}"))
    kb.add(types.InlineKeyboardButton("📖 راهنما", callback_data=f"pk:help:{chat_id}"))
    return kb
...
def poker_board_text(game: PokerGame):
    return " ".join(str(c) for c in game.get_community()) if game.community else "—"


def poker_status_text(game: PokerGame, extra_note: str = ""):
    board = poker_board_text(game)
    active = game.active_players()
    turn_uid = active[game.turn_index % len(active)]
    turn_name = game.player_names.get(turn_uid, turn_uid)
    owe = game.current_bet - game.round_contrib.get(str(turn_uid), 0)
    unit = game.unit_name()

    lines = [
        f"🃏 پوکر (تگزاس هولدم){' 🧪 تمرینی' if game.practice else ''} — pot: {game.pot} {unit}",
        f"دور: {POKER_ROUND_NAMES_FA[game.betting_round]}",
        f"کارت‌های میز: {board}",
        f"شرط این دور: {game.current_bet}",
    ]
    if extra_note:
        lines.append(extra_note)
    if owe > 0:
        lines.append(f"\n🎯 نوبت: {turn_name} — باید {owe} {unit} بیاره (Call) یا Raise بزنه.")
    else:
        lines.append(f"\n🎯 نوبت: {turn_name} — می‌تونه Check یا Raise بزنه.")
    lines.append("برای Raise: رو همین پیام ریپلای کن و فقط مبلغ رو بنویس.")
    return "\n".join(lines)


# ---------- دستورها ----------

@bot.message_handler(commands=["پوکر"])
@group_only
def cmd_poker(message):
    chat_id = message.chat.id
    existing = load_poker_game(chat_id)
    if existing and existing.phase != "done":
        bot.reply_to(message, "یه بازی پوکر همین الان تو این گروه در جریانه. اگه گیر کرده، /لغو_پوکر رو بزن.")
        return

    arg = message.text.split(maxsplit=1)[1].strip() if len(message.text.split(maxsplit=1)) > 1 else ""
    practice = False
    ante = POKER_ANTE
    if arg in ("تمرینی", "تمرین"):
        practice = True
        # باگ قبلی: اینجا ante رو برابر کل موجودی اولیه‌ی تمرینی (POKER_PRACTICE_BANKROLL)
        # می‌ذاشتیم، یعنی همون اول کل موجودی بابت ante صفر می‌شد و رایز کردن
        # همیشه «امتیاز کافی نداری» می‌داد. ante باید مثل حالت عادی یه بخش کوچیک باشه.
        ante = POKER_ANTE
    elif arg.isdigit() and int(arg) > 0:
        ante = int(arg)

    game = PokerGame(chat_id=chat_id, ante=ante, practice=practice)
    save_poker_game(game)

    unit = "امتیاز تمرینی" if practice else "شیهه‌پوینت"
    mode_line = "🧪 حالت تمرینی — بدون شیهه‌پوینت واقعی، خود بازی بهت موجودی می‌ده.\n" if practice else ""
    bot.send_message(
        chat_id,
        f"🃏 میز پوکر باز شد! (تگزاس هولدم، ورودی هر نفر {ante} {unit})\n"
        f"{mode_line}"
        f"بین {POKER_MIN_PLAYERS} تا {POKER_MAX_PLAYERS} نفر می‌تونن بازی کنن.\n\n"
        f"میز (۰/{POKER_MAX_PLAYERS}):",
        reply_markup=poker_kb_lobby(chat_id, can_start=False, ante=ante, practice=practice),
    )


@bot.message_handler(commands=["لغو_پوکر"])
@group_only
def cmd_cancel_poker(message):
    chat_id = message.chat.id
    game = load_poker_game(chat_id)
    if not game:
        bot.reply_to(message, "بازی فعالی برای لغو کردن نیست.")
        return
    if game.phase != "waiting":
        for uid in game.players:
            poker_pay(game, uid, game.ante)
    delete_poker_game(chat_id)
    bot.reply_to(message, "بازی پوکر لغو شد.")


# ---------- هندلر کال‌بک ----------

def handle_poker_callback(call):
    data = call.data
    parts = data.split(":")
    action = parts[1]
    chat_id = int(parts[2])
    ...
    if action == "join":
        poker_handle_join(call, chat_id)
    elif action == "start":
        poker_handle_start(call, chat_id)
    elif action == "check":
        poker_handle_check(call, chat_id)
    elif action == "call":
        poker_handle_call(call, chat_id)
    elif action == "fold":
        poker_handle_fold(call, chat_id)
    elif action == "help":
        bot.answer_callback_query(call.id, POKER_HELP_TEXT, show_alert=True)
    elif action == "raiseinfo":
        bot.answer_callback_query(
            call.id,
            "برای رایز، رو پیام بازی تو گروه ریپلای کن و فقط مبلغش رو بنویس (مثلاً: 30)",
            show_alert=True,
        )
    else:
        bot.answer_callback_query(call.id)
...
def poker_handle_fold(call, chat_id):
    user_id = call.from_user.id
    game = load_poker_game(chat_id)
    if not game or game.phase != "betting":
        bot.answer_callback_query(call.id, "این دست دیگه فعال نیست.", show_alert=True)
        return
    if user_id != poker_current_turn_player(game):
        bot.answer_callback_query(call.id, "الان نوبت تو نیست.", show_alert=True)
        return

    game.folded.append(user_id)
    save_poker_game(game)
    bot.answer_callback_query(call.id, "فولد کردی 🏳️")
    poker_after_fold(chat_id, game, user_id)


def poker_handle_join(call, chat_id):
    user = call.from_user
    game = load_poker_game(chat_id)
    ...
    if user.id in game.players:
        bot.answer_callback_query(call.id, "تو که قبلاً نشستی سر میز!", show_alert=True)
        return
    if len(game.players) >= POKER_MAX_PLAYERS:
        bot.answer_callback_query(call.id, "میز پره.", show_alert=True)
        return

    if not hokm_can_pm(user.id):
        bot.answer_callback_query(
            call.id, "اول باید پیوی ربات رو باز کنی (پایین گروه یه دکمه فرستادم).", show_alert=True
        )
        bot.send_message(
            chat_id,
            f"{user.first_name} عزیز، چون کارتاتو باید خصوصی بفرستم، اول این دکمه رو بزن "
    ...
    if game.practice:
        if str(user.id) not in game.practice_balance:
            game.practice_balance[str(user.id)] = POKER_PRACTICE_BANKROLL
    else:
        u = get_user_row(user.id)
        if not u or (u.get("shihe_points") or 0) < game.ante:
            bot.answer_callback_query(
                call.id, f"برای نشستن سر میز حداقل {game.ante} شیهه‌پوینت لازم داری.", show_alert=True
            )
            return
    ...
    names = " · ".join(game.get_name(p) for p in game.players)
    can_start = len(game.players) >= POKER_MIN_PLAYERS
    full = len(game.players) >= POKER_MAX_PLAYERS
    unit = game.unit_name()
    bot.edit_message_text(
        f"🃏 میز پوکر (تگزاس هولدم، ورودی هر نفر {game.ante} {unit})\n"
        f"میز ({len(game.players)}/{POKER_MAX_PLAYERS}): {names}",
        chat_id, call.message.message_id,
        reply_markup=poker_kb_lobby(chat_id, can_start=can_start and not full, ante=game.ante, practice=game.practice) if not full else None,
    )

    if full:
        poker_start_hand(chat_id)


def poker_handle_start(call, chat_id):
    game = load_poker_game(chat_id)
    if not game or game.phase != "waiting":
        bot.answer_callback_query(call.id, "این بازی دیگه فعال نیست.", show_alert=True)
        return
    if call.from_user.id not in game.players:
        bot.answer_callback_query(call.id, "فقط کسی که سر میز نشسته می‌تونه شروع کنه.", show_alert=True)
        return
    if len(game.players) < POKER_MIN_PLAYERS:
        bot.answer_callback_query(call.id, f"حداقل {POKER_MIN_PLAYERS} نفر لازمه.", show_alert=True)
        return
...
def poker_start_hand(chat_id):
    game = load_poker_game(chat_id)
    if not game or game.phase != "waiting":
        return

    for uid in game.players:
        if poker_get_balance(game, uid) < game.ante:
            bot.send_message(
                chat_id,
                f"{game.player_names.get(uid, uid)} دیگه {game.ante} {game.unit_name()} نداره، بازی لغو شد."
            )
            delete_poker_game(chat_id)
            return
    ...
    game.pot = game.ante * len(game.players)
    for uid in game.players:
        hole = [deck.pop() for _ in range(2)]
        game.hole_cards[str(uid)] = [c.to_str() for c in hole]
    game.deck = [c.to_str() for c in deck]
    game.phase = "betting"
    ...
    bot.send_message(
        chat_id,
        f"🃏 دست شروع شد! وجه رو میز: {game.pot} {game.unit_name()}\n"
        f"کارت‌های خصوصیت (۲ تا) تو پیوی رفت. حالا نوبتی بازی می‌کنیم."
    )

    for uid in game.players:
        hole = game.get_hole(uid)
        bot.send_message(uid, f"🂠 کارت‌های دستت:\n{' '.join(str(c) for c in hole)}")

    poker_send_turn_prompt(chat_id)


def poker_send_turn_prompt(chat_id, extra_note=""):
    game = load_poker_game(chat_id)
    active = game.active_players()
    turn_uid = active[game.turn_index % len(active)]
    owe = game.current_bet - game.round_contrib.get(str(turn_uid), 0)
    ...
    msg = bot.send_message(
        chat_id,
        poker_status_text(game, extra_note),
        reply_markup=poker_kb_turn(chat_id, owe),
    )
    game.game_message_id = msg.message_id
    save_poker_game(game)


def poker_update_turn_prompt(chat_id, extra_note=""):
    """پیام بازی رو ادیت می‌کنه به‌جای فرستادن پیام جدید، طبق خواسته."""
    game = load_poker_game(chat_id)
    active = game.active_players()
    turn_uid = active[game.turn_index % len(active)]
    owe = game.current_bet - game.round_contrib.get(str(turn_uid), 0)
    ...
    if game.game_message_id:
        try:
            bot.edit_message_text(
                poker_status_text(game, extra_note),
                chat_id, game.game_message_id,
                reply_markup=poker_kb_turn(chat_id, owe),
            )
            return
        except Exception:
            pass
    # اگه ادیت نشد (مثلاً پیام خیلی قدیمیه)، یه پیام جدید بفرست
    poker_send_turn_prompt(chat_id, extra_note)


def poker_current_turn_player(game: PokerGame):
    active = game.active_players()
    return active[game.turn_index % len(active)]


def poker_advance_turn(game: PokerGame):
    """نفر بعدی (فعال و فولد‌نکرده) رو برمی‌گردونه."""
    active = game.active_players()
    game.turn_index = (game.turn_index + 1) % len(active)


def poker_round_complete(game: PokerGame) -> bool:
    active = game.active_players()
    if len(active) <= 1:
        return True
    for p in active:
        if p not in game.acted:
            return False
        if game.round_contrib.get(str(p), 0) != game.current_bet:
            return False
    return True


def poker_handle_check(call, chat_id):
    user_id = call.from_user.id
    game = load_poker_game(chat_id)
    if not game or game.phase != "betting":
        bot.answer_callback_query(call.id, "این دست دیگه فعال نیست.", show_alert=True)
    ...
    if user_id != poker_current_turn_player(game):
        bot.answer_callback_query(call.id, "الان نوبت تو نیست.", show_alert=True)
        return
    owe = game.current_bet - game.round_contrib.get(str(user_id), 0)
    if owe > 0:
        bot.answer_callback_query(call.id, "شرط باز مونده، باید Call یا Raise کنی.", show_alert=True)
        return

    if user_id not in game.acted:
        game.acted.append(user_id)
    bot.answer_callback_query(call.id, "Check ✋")
    poker_advance_and_continue(chat_id, game, f"✋ {game.player_names.get(user_id, user_id)} چک کرد.")


def poker_handle_call(call, chat_id):
    user_id = call.from_user.id
    game = load_poker_game(chat_id)
    if not game or game.phase != "betting":
        bot.answer_callback_query(call.id, "این دست دیگه فعال نیست.", show_alert=True)
    ...
    owe = game.current_bet - game.round_contrib.get(str(user_id), 0)
    if owe <= 0:
        bot.answer_callback_query(call.id, "چیزی برای Call نمونده، Check بزن.", show_alert=True)
        return

    if poker_get_balance(game, user_id) < owe:
        # شیهه‌پوینت/امتیاز کافی نیست — خودکار فولد می‌شه
        game.folded.append(user_id)
        save_poker_game(game)
        bot.answer_callback_query(call.id, f"{game.unit_name()} کافی نداری، فولد شدی.", show_alert=True)
        poker_after_fold(chat_id, game, user_id)
        return

    poker_pay(game, user_id, -owe)
    game.pot += owe
    game.round_contrib[str(user_id)] = game.current_bet
    if user_id not in game.acted:
        game.acted.append(user_id)
    bot.answer_callback_query(call.id, f"Call {owe} ✅")
    poker_advance_and_continue(chat_id, game, f"✅ {game.player_names.get(user_id, user_id)} با {owe} کال کرد.")


def poker_advance_and_continue(chat_id, game, note):
    save_poker_game(game)
    if poker_round_complete(game):
        poker_next_round_or_showdown(chat_id, note)
    else:
        poker_advance_turn(game)
        save_poker_game(game)
        poker_update_turn_prompt(chat_id, note)


def poker_after_fold(chat_id, game, folded_user_id):
    active = game.active_players()
    if len(active) == 1:
        poker_payout_single_winner(chat_id, game, active[0])
        return
    # ایندکس نوبت رو با توجه به فولد جدید تنظیم کن و ادامه بده
    if game.turn_index >= len(active):
        game.turn_index = 0
    save_poker_game(game)
    if poker_round_complete(game):
        poker_next_round_or_showdown(chat_id, f"{game.player_names.get(folded_user_id, folded_user_id)} فولد کرد.")
    else:
        poker_update_turn_prompt(chat_id, f"{game.player_names.get(folded_user_id, folded_user_id)} فولد کرد.")


def poker_handle_raise_reply(message, game, user_id, amount):
    chat_id = game.chat_id
    if amount <= 0:
        bot.reply_to(message, "مبلغ رایز باید یه عدد مثبت باشه.")
        return

    already = game.round_contrib.get(str(user_id), 0)
    owe_to_call = game.current_bet - already
    need = owe_to_call + amount
    if poker_get_balance(game, user_id) < need:
        bot.reply_to(message, f"{game.unit_name()} کافی نداری. برای این رایز {need} تا لازمه.")
        return

    poker_pay(game, user_id, -need)
    game.pot += need
    ...
    bot.reply_to(message, f"📈 رایز شد! شرط این دور الان {game.current_bet} شیهه‌پوینته.")
    poker_advance_turn(game)
    save_poker_game(game)
    poker_update_turn_prompt(chat_id, f"📈 {game.player_names.get(user_id, user_id)} رایز کرد به {game.current_bet}.")


def poker_next_round_or_showdown(chat_id, note=""):
    game = load_poker_game(chat_id)
    active = game.active_players()
    ...
    if game.betting_round == "river":
        poker_showdown(chat_id)
        return

    # مرحله‌ی بعد: کارت‌های میز رو باز کن
    deck = [Card.from_str(s) for s in game.deck]
    if game.betting_round == "preflop":
        new_cards = [deck.pop() for _ in range(3)]
        game.betting_round = "flop"
    elif game.betting_round == "flop":
        new_cards = [deck.pop()]
    ...
    game.community.extend(c.to_str() for c in new_cards)
    game.deck = [c.to_str() for c in deck]
    game.current_bet = 0
    game.round_contrib = {}
    game.acted = []
    ...
    board = " ".join(str(c) for c in game.get_community())
    poker_send_turn_prompt(
        chat_id,
        f"{note}\n\n🃏 مرحله‌ی {POKER_ROUND_NAMES_FA[game.betting_round]}! کارت‌های میز: {board}".strip(),
    )


def poker_payout_single_winner(chat_id, game, winner_id):
    poker_pay(game, winner_id, game.pot)
    bot.send_message(
        chat_id,
        f"🏆 همه فولد کردن، {game.player_names.get(winner_id, winner_id)} "
        f"بدون رو کردن کارت‌ها {game.pot} {game.unit_name()} رو برد!",
    )
    delete_poker_game(chat_id)


def poker_showdown(chat_id):
    game = load_poker_game(chat_id)
    if not game:
        return

    community = game.get_community()
    active = game.active_players()
    results = []
    for uid in active:
        hole = game.get_hole(uid)
        score = poker_best_of_seven(hole + community)
        results.append((uid, hole, score))
    results.sort(key=lambda r: r[2], reverse=True)

    best_score = results[0][2]
    winners = [uid for uid, _, score in results if score == best_score]

    share = game.pot // len(winners)
    leftover = game.pot - share * len(winners)
    for idx, uid in enumerate(winners):
        amount = share + (leftover if idx == 0 else 0)
        poker_pay(game, uid, amount)

    board = " ".join(str(c) for c in community)
    lines = [f"🃏 نتیجه‌ی دست! کارت‌های میز: {board}\n"]
    for uid, hole, score in results:
        tag = " 🏆" if uid in winners else ""
        lines.append(
            f"{game.player_names.get(uid, uid)}: {' '.join(str(c) for c in hole)} "
            f"— {poker_hand_name(score)}{tag}"
        )

    if len(winners) == 1:
        win_name = game.player_names.get(winners[0], winners[0])
        lines.append(f"\n🏆 برنده: {win_name} — {game.pot} {game.unit_name()} برد!")
    else:
        win_names = " و ".join(game.player_names.get(w, w) for w in winners)
        lines.append(f"\n🏆 مساوی شد بین {win_names} — وجه بین‌شون تقسیم شد!")
...
@bot.message_handler(
    func=lambda m: m.reply_to_message is not None and m.text and m.text.strip().lstrip("-").isdigit()
)
def handle_digit_reply(message):
    chat_id = message.chat.id
    ...
    # پوکر: رایز وسط دست
    game = load_poker_game(chat_id)
    if game and game.phase == "betting" and game.game_message_id == reply_id:
        user_id = message.from_user.id
        if user_id != poker_current_turn_player(game):
            bot.reply_to(message, "الان نوبت تو نیست.")
            return
        poker_handle_raise_reply(message, game, user_id, amount)
        return

    # بلک‌جک: تعیین شرط دلخواه قبل از ورود به میز
    bj = load_blackjack_game(chat_id)
    if bj and bj.phase == "waiting" and bj.lobby_message_id == reply_id:
        if amount <= 0:
            bot.reply_to(message, "مبلغ شرط باید یه عدد مثبت باشه.")
            return
        bj.pending_bets[str(message.from_user.id)] = amount
        save_blackjack_game(bj)
        unit = "امتیاز تمرینی" if bj.practice else "شیهه‌پوینت"
        bot.reply_to(message, f"باشه، شرطت {amount} {unit} ثبت شد — حالا «ورود به میز» رو بزن.")
        return

    # گمبل: مبلغ دلخواه شرط (اسلات/شیر‌خط/بیشتر‌کمتر/تاس/مین‌یاب)
    gamble_custom = _gamble_custom_pending.get((chat_id, message.from_user.id))
    if gamble_custom and gamble_custom["message_id"] == reply_id:
        ok, bet_or_err = _validate_gamble_bet(message.text.strip())
        if not ok:
            bot.reply_to(message, bet_or_err)
            return
        bet = bet_or_err
        del _gamble_custom_pending[(chat_id, message.from_user.id)]

        game = gamble_custom["game"]
        extra = gamble_custom.get("extra")
        user_id = message.from_user.id
        if game == "slot":
            ok2, text, kb2 = start_slot_spin(chat_id, user_id, bet)
            sent = bot.reply_to(message, text, reply_markup=kb2)
            if ok2:
                attach_slot_message_id(chat_id, user_id, sent.message_id)
        elif game == "coin":
            _, text, kb = start_coin_streak(chat_id, user_id, extra, bet)
            bot.reply_to(message, text, reply_markup=kb)
        elif game == "dice":
            _, text, kb = start_dice_streak(chat_id, user_id, extra, bet)
            bot.reply_to(message, text, reply_markup=kb)
        elif game == "hilo":
            _, text, kb = start_hilo(chat_id, user_id, bet)
            bot.reply_to(message, text, reply_markup=kb)
        elif game == "mines":
            _, text, kb = start_mines(chat_id, user_id, bet, int(extra))
            bot.reply_to(message, text, reply_markup=kb)
        return
...
#                  🂡  بازی بلک‌جک  🂡
# =========================================================
# هرکی جدا داره در برابر دیلر (خونه) بازی می‌کنه، نه در برابر هم.
# همه‌ی کارت‌ها (به‌جز کارت دومِ دیلر) از اول تو گروه معلومه — نیازی
# به پیوی نیست. نوبتی هرکی Hit/Stand می‌زنه، آخرش دیلر می‌کِشه و
# نتیجه‌ها با هم مقایسه می‌شن. شرط‌ها با شیهه‌پوینت حساب می‌شه.
...
def bj_hand_value(cards: list[Card]) -> int:
    total = 0
    aces = 0
    for c in cards:
        if c.rank == "A":
...
def bj_is_blackjack(cards: list[Card]) -> bool:
    return len(cards) == 2 and bj_hand_value(cards) == 21


def bj_make_shoe():
    """دو دست کامل رو با هم قاطی می‌کنیم تا برای چند نفره جا کم نیاره."""
    return make_deck() + make_deck()


@dataclass
class BlackjackGame:
    chat_id: int
    phase: str = "waiting"                              # waiting, playing, done
    players: list[int] = field(default_factory=list)
    player_names: dict[int, str] = field(default_factory=dict)
    hands: dict[str, list[str]] = field(default_factory=dict)
    busted: list[int] = field(default_factory=list)
    stood: list[int] = field(default_factory=list)       # شامل بلک‌جک‌های طبیعی هم می‌شه
    natural: list[int] = field(default_factory=list)     # کیا بلک‌جک طبیعی گرفتن (پرداخت ۳به۲)
    dealer_hand: list[str] = field(default_factory=list)
    dealer_hidden: bool = True
    deck: list[str] = field(default_factory=list)
    turn_index: int = 0
    game_message_id: Optional[int] = None
    lobby_message_id: Optional[int] = None                # پیام لابی (برای ریپلای شرط دلخواه)
    default_bet: int = 50                                 # شرط پیش‌فرض این میز
    bets: dict[str, int] = field(default_factory=dict)    # str(user_id) -> شرط خودش
    pending_bets: dict[str, int] = field(default_factory=dict)  # شرط‌های اعلام‌شده قبل از ورود به میز
    practice: bool = False                                # حالت تمرینی — بدون شیهه‌پوینت واقعی
    practice_balance: dict[str, int] = field(default_factory=dict)
    ...
    @staticmethod
    def from_dict(d):
        game = BlackjackGame(**d)
        game.player_names = {int(k): v for k, v in game.player_names.items()}
        return game

    def pending_players(self):
        """کسایی که هنوز نه فولد (باخت) شدن نه ایستادن — باید تصمیم بگیرن."""
        return [p for p in self.players if p not in self.busted and p not in self.stood]

    def get_hand(self, user_id) -> list[Card]:
        return [Card.from_str(s) for s in self.hands[str(user_id)]]

    def get_dealer_hand(self) -> list[Card]:
        return [Card.from_str(s) for s in self.dealer_hand]
...
BJ_PRACTICE_BANKROLL = 100  # موجودی خیالی هر نفر تو حالت تمرینی


def bj_get_balance(game: BlackjackGame, user_id) -> int:
    if game.practice:
        return game.practice_balance.get(str(user_id), 0)
    row = get_user_row(user_id)
    return (row.get("shihe_points") or 0) if row else 0
...
def bj_pay(game: BlackjackGame, user_id, delta: int) -> None:
    if game.practice:
        key = str(user_id)
        game.practice_balance[key] = game.practice_balance.get(key, 0) + delta
    else:
...
def bj_bet_of(game: BlackjackGame, user_id) -> int:
    return game.bets.get(str(user_id), game.default_bet)


def init_blackjack_tables():
    with db_lock:
        conn = get_conn()
        c = conn.cursor()
        c.execute("""
...
def save_blackjack_game(game: BlackjackGame):
    ...
    with db_lock:
        conn = get_conn()
        conn.execute(
            "INSERT INTO blackjack_games (chat_id, state) VALUES (?, ?) "
            "ON CONFLICT(chat_id) DO UPDATE SET state=excluded.state",
            (game.chat_id, state_json)
        )
        conn.commit()
...
def load_blackjack_game(chat_id):
    with db_lock:
        conn = get_conn()
        row = conn.execute("SELECT state FROM blackjack_games WHERE chat_id=?", (chat_id,)).fetchone()
        conn.close()
    if not row:
        return None
    return BlackjackGame.from_dict(json.loads(row["state"]))


def delete_blackjack_game(chat_id):
    with db_lock:
        conn = get_conn()
        conn.execute("DELETE FROM blackjack_games WHERE chat_id=?", (chat_id,))
        conn.commit()
        conn.close()
...
BJ_HELP_TEXT = (
    "🂡 Blackjack basics:\n"
    "2-10 = face value, J/Q/K = 10, Ace = 1 or 11\n"
    "Goal: get closer to 21 than the dealer without going over.\n"
    "Dealer hits until 17+. Natural 21 (2 cards) pays 3:2."
)


def bj_kb_lobby(chat_id, can_start, default_bet=None, practice=False):
    amount = default_bet if default_bet is not None else BJ_BET
    unit = "امتیاز تمرینی" if practice else "شیهه‌پوینت"
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(
        f"🪑 ورود به میز (شرط: {amount} {unit})", callback_data=f"bj:join:{chat_id}"
    ))
    if can_start:
        kb.add(types.InlineKeyboardButton("🎬 شروع دست", callback_data=f"bj:start:{chat_id}"))
    kb.add(types.InlineKeyboardButton("📖 راهنما", callback_data=f"bj:help:{chat_id}"))
    return kb


def bj_kb_turn(chat_id):
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(types.InlineKeyboardButton("🃏 Hit", callback_data=f"bj:hit:{chat_id}"))
    kb.add(types.InlineKeyboardButton("✋ Stand", callback_data=f"bj:stand:{chat_id}"))
    kb.add(types.InlineKeyboardButton("📖 راهنما", callback_data=f"bj:help:{chat_id}"))
    return kb
...
def bj_status_text(game: BlackjackGame, extra_note: str = ""):
    lines = [f"🂡 بلک‌جک{' 🧪 تمرینی' if game.practice else ''}\n"]

    dealer_hand = game.get_dealer_hand()
    if game.dealer_hidden and len(dealer_hand) >= 1:
        lines.append(f"دیلر: {dealer_hand[0]} 🂠")
    else:
        lines.append(f"دیلر: {' '.join(str(c) for c in dealer_hand)} = {bj_hand_value(dealer_hand)}")

    lines.append("")
    for uid in game.players:
        hand = game.get_hand(uid)
        val = bj_hand_value(hand)
        tag = ""
        if uid in game.natural:
            tag = " 🎉 بلک‌جک!"
        elif uid in game.busted:
            tag = " ❌ باخت (بیشتر از ۲۱)"
        elif uid in game.stood:
            tag = " ✋ ایستاد"
        lines.append(f"{game.get_name(uid)} (شرط {bj_bet_of(game, uid)}): {' '.join(str(c) for c in hand)} = {val}{tag}")

    if extra_note:
        lines.append(f"\n{extra_note}")

    pending = game.pending_players()
    if pending:
        turn_uid = pending[game.turn_index % len(pending)]
        lines.append(f"\n🎯 نوبت: {game.get_name(turn_uid)} — Hit یا Stand؟")
...
@bot.message_handler(commands=["بلک_جک"])
@group_only
def cmd_blackjack(message):
    chat_id = message.chat.id
    existing = load_blackjack_game(chat_id)
    if existing and existing.phase != "done":
        bot.reply_to(message, "یه بازی بلک‌جک همین الان تو این گروه در جریانه. اگه گیر کرده، /لغو_بلک_جک رو بزن.")
        return

    parts = message.text.split(maxsplit=1)
    arg = parts[1].strip() if len(parts) > 1 else ""
    practice = arg in ("تمرینی", "تمرین")
    default_bet = BJ_PRACTICE_BANKROLL if practice else BJ_BET

    game = BlackjackGame(chat_id=chat_id, practice=practice, default_bet=default_bet)
    ...
    sent = bot.send_message(
        chat_id,
        f"🂡 میز بلک‌جک باز شد! (شرط پیش‌فرض هر نفر: {default_bet} {unit})\n"
        f"{mode_line}"
        f"می‌خوای شرط دلخواه بذاری؟ رو همین پیام ریپلای کن و فقط عددشو بفرست، بعد «ورود به میز» رو بزن.\n"
        f"تا {BJ_MAX_PLAYERS} نفر می‌تونن بازی کنن، هرکی جدا در برابر دیلره.\n\n"
        f"میز (۰/{BJ_MAX_PLAYERS}):",
        reply_markup=bj_kb_lobby(chat_id, can_start=False, default_bet=default_bet, practice=practice),
    )
    game.lobby_message_id = sent.message_id
    save_blackjack_game(game)


@bot.message_handler(commands=["لغو_بلک_جک"])
@group_only
def cmd_cancel_blackjack(message):
    chat_id = message.chat.id
    game = load_blackjack_game(chat_id)
    if not game:
        bot.reply_to(message, "بازی فعالی برای لغو کردن نیست.")
        return
    if game.phase != "waiting":
        for uid in game.players:
            bj_pay(game, uid, bj_bet_of(game, uid))
    delete_blackjack_game(chat_id)
    bot.reply_to(message, "بازی بلک‌جک لغو شد.")


# ---------- هندلر کال‌بک ----------

def handle_blackjack_callback(call):
    data = call.data
    parts = data.split(":")
    action = parts[1]
    chat_id = int(parts[2])
    ...
    if action == "join":
        bj_handle_join(call, chat_id)
    elif action == "start":
        bj_handle_start(call, chat_id)
    elif action == "hit":
        bj_handle_hit(call, chat_id)
    elif action == "stand":
        bj_handle_stand(call, chat_id)
    elif action == "help":
        bot.answer_callback_query(call.id, BJ_HELP_TEXT, show_alert=True)
    else:
        bot.answer_callback_query(call.id)


def bj_handle_join(call, chat_id):
    user = call.from_user
    game = load_blackjack_game(chat_id)
    if not game or game.phase != "waiting":
        bot.answer_callback_query(call.id, "این بازی دیگه فعال نیست.", show_alert=True)
    ...
    if user.id in game.players:
        bot.answer_callback_query(call.id, "تو که قبلاً نشستی سر میز!", show_alert=True)
        return
    if len(game.players) >= BJ_MAX_PLAYERS:
        bot.answer_callback_query(call.id, "میز پره.", show_alert=True)
        return

    ensure_user(user.id, user.username or user.first_name or "")
    bet = game.pending_bets.get(str(user.id), game.default_bet)

    if game.practice:
        if str(user.id) not in game.practice_balance:
            game.practice_balance[str(user.id)] = BJ_PRACTICE_BANKROLL
        if bj_get_balance(game, user.id) < bet:
            bot.answer_callback_query(call.id, f"موجودی تمرینیت کمتر از {bet} تاست.", show_alert=True)
            return
    else:
        u = get_user_row(user.id)
        if not u or (u.get("shihe_points") or 0) < bet:
            bot.answer_callback_query(call.id, f"برای این شرط حداقل {bet} شیهه‌پوینت لازم داری.", show_alert=True)
            return

    game.players.append(user.id)
    game.player_names[user.id] = user.first_name
    game.bets[str(user.id)] = bet
    save_blackjack_game(game)
    bot.answer_callback_query(call.id, f"وارد میز شدی، {user.first_name}! (شرط: {bet})")

    names = " · ".join(f"{game.get_name(p)} ({bj_bet_of(game, p)})" for p in game.players)
    can_start = len(game.players) >= BJ_MIN_PLAYERS
    full = len(game.players) >= BJ_MAX_PLAYERS
    unit = game.unit_name()
    bot.edit_message_text(
        f"🂡 میز بلک‌جک (شرط پیش‌فرض: {game.default_bet} {unit})\n"
        f"میز ({len(game.players)}/{BJ_MAX_PLAYERS}): {names}",
        chat_id, call.message.message_id,
        reply_markup=bj_kb_lobby(chat_id, can_start=can_start and not full, default_bet=game.default_bet, practice=game.practice) if not full else None,
    )

    if full:
        bj_start_hand(chat_id)


def bj_handle_start(call, chat_id):
    game = load_blackjack_game(chat_id)
    if not game or game.phase != "waiting":
        bot.answer_callback_query(call.id, "این بازی دیگه فعال نیست.", show_alert=True)
        return
    ...
    if len(game.players) < BJ_MIN_PLAYERS:
        bot.answer_callback_query(call.id, f"حداقل {BJ_MIN_PLAYERS} نفر لازمه.", show_alert=True)
        return
...
def bj_start_hand(chat_id):
    game = load_blackjack_game(chat_id)
    if not game or game.phase != "waiting":
        return

    for uid in game.players:
        bet = bj_bet_of(game, uid)
        if bj_get_balance(game, uid) < bet:
            bot.send_message(
                chat_id,
                f"{game.player_names.get(uid, uid)} دیگه {bet} {game.unit_name()} نداره، بازی لغو شد."
            )
            delete_blackjack_game(chat_id)
            return
    ...
    for uid in game.players:
        ...
        game.hands[str(uid)] = [c.to_str() for c in hand]
        if bj_is_blackjack(hand):
            game.natural.append(uid)
            game.stood.append(uid)

    game.dealer_hand = [deck.pop().to_str(), deck.pop().to_str()]
    game.deck = [c.to_str() for c in deck]
    game.phase = "playing"
    game.turn_index = 0
    save_blackjack_game(game)
    ...
    pending = game.pending_players()
    if not pending:
        # همه بلک‌جک طبیعی داشتن، مستقیم بریم سراغ دیلر
        bj_dealer_play(chat_id)
        return

    msg = bot.send_message(chat_id, bj_status_text(game), reply_markup=bj_kb_turn(chat_id))
    game.game_message_id = msg.message_id
    save_blackjack_game(game)


def bj_current_turn_player(game: BlackjackGame):
    pending = game.pending_players()
    return pending[game.turn_index % len(pending)]


def bj_update_turn_prompt(chat_id, extra_note=""):
    game = load_blackjack_game(chat_id)
    if game.game_message_id:
        try:
            bot.edit_message_text(
                bj_status_text(game, extra_note),
                chat_id, game.game_message_id,
                reply_markup=bj_kb_turn(chat_id),
            )
            return
        except Exception:
            pass
    msg = bot.send_message(chat_id, bj_status_text(game, extra_note), reply_markup=bj_kb_turn(chat_id))
    game.game_message_id = msg.message_id
    save_blackjack_game(game)


def bj_handle_hit(call, chat_id):
    user_id = call.from_user.id
    game = load_blackjack_game(chat_id)
    if not game or game.phase != "playing":
        bot.answer_callback_query(call.id, "این دست دیگه فعال نیست.", show_alert=True)
        return
    if user_id != bj_current_turn_player(game):
        bot.answer_callback_query(call.id, "الان نوبت تو نیست.", show_alert=True)
        return

    deck = [Card.from_str(s) for s in game.deck]
    new_card = deck.pop()
    hand = game.hands[str(user_id)]
    hand.append(new_card.to_str())
    game.hands[str(user_id)] = hand
    game.deck = [c.to_str() for c in deck]

    value = bj_hand_value([Card.from_str(s) for s in hand])
    note = f"🃏 {game.get_name(user_id)} یه کارت کشید: {new_card}"
    if value > 21:
        game.busted.append(user_id)
        note += f" — باخت ({value})!"
        bot.answer_callback_query(call.id, f"باختی ({value})، بیشتر از ۲۱ شد.", show_alert=True)
        save_blackjack_game(game)
        bj_advance_or_dealer(chat_id, note)
    else:
        bot.answer_callback_query(call.id, f"کارتت: {new_card} (مجموع {value})")
        save_blackjack_game(game)
        bj_update_turn_prompt(chat_id, note)


def bj_handle_stand(call, chat_id):
    user_id = call.from_user.id
    game = load_blackjack_game(chat_id)
    if not game or game.phase != "playing":
        bot.answer_callback_query(call.id, "این دست دیگه فعال نیست.", show_alert=True)
    ...
    game.stood.append(user_id)
    save_blackjack_game(game)
    bot.answer_callback_query(call.id, "Stand ✋")
    bj_advance_or_dealer(chat_id, f"✋ {game.get_name(user_id)} ایستاد.")


def bj_advance_or_dealer(chat_id, note):
    game = load_blackjack_game(chat_id)
    pending = game.pending_players()
    if not pending:
        bj_dealer_play(chat_id, note)
    else:
        game.turn_index = 0
        save_blackjack_game(game)
        bj_update_turn_prompt(chat_id, note)
...
def bj_dealer_play(chat_id, note=""):
    game = load_blackjack_game(chat_id)
    game.dealer_hidden = False

    # اگه همه بازیکن‌ها باختن، دیلر لازم نیست بکشه
    any_alive = any(uid not in game.busted for uid in game.players)
    dealer_hand = game.get_dealer_hand()
    deck = [Card.from_str(s) for s in game.deck]

    if any_alive:
        while bj_hand_value(dealer_hand) < 17:
            dealer_hand.append(deck.pop())

    game.dealer_hand = [c.to_str() for c in dealer_hand]
    game.deck = [c.to_str() for c in deck]
    game.phase = "done"
    save_blackjack_game(game)

    dealer_value = bj_hand_value(dealer_hand)
    dealer_busted = dealer_value > 21

    lines = [f"🂡 نتیجه‌ی دست!\n"]
    lines.append(f"دیلر: {' '.join(str(c) for c in dealer_hand)} = {dealer_value}" + (" (باخت!)" if dealer_busted else ""))
    lines.append("")

    for uid in game.players:
        hand = game.get_hand(uid)
        value = bj_hand_value(hand)
        name = game.get_name(uid)
        bet = bj_bet_of(game, uid)
        unit = game.unit_name()

        if uid in game.busted:
            lines.append(f"{name}: {value} — ❌ باخت، {bet} {unit} از دست رفت.")
            continue

        if uid in game.natural:
            if dealer_value == 21 and len(dealer_hand) == 2:
                bj_pay(game, uid, bet)
                lines.append(f"{name}: بلک‌جک طبیعی — دیلر هم بلک‌جک داشت، مساوی، شرط برگشت.")
            else:
                win = int(bet * 2.5)
                bj_pay(game, uid, win)
                lines.append(f"{name}: بلک‌جک طبیعی! 🎉 — {win} {unit} برد.")
            continue

        if dealer_busted or value > dealer_value:
            bj_pay(game, uid, bet * 2)
            lines.append(f"{name}: {value} — ✅ برد، {bet * 2} {unit} گرفت.")
        elif value == dealer_value:
            bj_pay(game, uid, bet)
            lines.append(f"{name}: {value} — 🤝 مساوی، شرط برگشت.")
        else:
            lines.append(f"{name}: {value} — ❌ باخت، {bet} {unit} از دست رفت.")
...
# =========================================================
#              🎮  منوی مرکزی بازی‌ها  (/بازی)  🎮
# =========================================================
# یه نقطه‌ی ورود مشترک: اول می‌پرسه چند نفره، بعد بازی‌های همون تعداد
# نفرات رو نشون می‌ده. دستورهای /حکم و /نبرد_دریایی هم مثل قبل مستقیم
# کار می‌کنن — این فقط یه راه ورود ساده‌تر و یکپارچه‌ست.

def game_kb_playercount():
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        types.InlineKeyboardButton("👥 ۲ نفره", callback_data="game:count:2"),
        types.InlineKeyboardButton("👨‍👩‍👧‍👦 ۴ نفره", callback_data="game:count:4"),
    )
    kb.add(types.InlineKeyboardButton("🔙 برگشت به منو", callback_data="menu:main"))
    return kb


def game_kb_list(num_players):
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("🃏 حکم", callback_data=f"game:hokm:{num_players}"))
    if num_players == 2:
        kb.add(types.InlineKeyboardButton("🚢 نبرد دریایی", callback_data="game:battleship"))
    kb.add(types.InlineKeyboardButton("🃏 پوکر", callback_data="game:poker"))
    kb.add(types.InlineKeyboardButton("🂡 بلک‌جک", callback_data="game:blackjack"))
    return kb


@bot.message_handler(commands=["بازی"])
@group_only
def cmd_game_menu(message):
    bot.send_message(message.chat.id, "🎮 چند نفره بازی می‌کنین؟", reply_markup=game_kb_playercount())


def handle_game_menu_callback(call):
    data = call.data
    parts = data.split(":")
    action = parts[1]
    ...
    if action == "count":
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"🎮 بازی‌های {num_players}نفره:",
            call.message.chat.id, call.message.message_id,
            reply_markup=game_kb_list(num_players),
        )
    elif action == "hokm":
        num_players = int(parts[2])
        chat_id = call.message.chat.id
        existing = load_hokm_game(chat_id)
        if existing and existing.phase != Phase.MATCH_OVER:
            bot.answer_callback_query(call.id, "یه بازی حکم همین الان تو این گروه در جریانه.", show_alert=True)
            return
        game = HokmGame(chat_id=chat_id, num_players=num_players)
        save_hokm_game(game)
        bot.answer_callback_query(call.id)
        ...
        bot.edit_message_text(
            f"🚢 میز نبرد دریایی #{table_id} باز شد!\n"
            f"منتظر نفر دومیم — هرکی می‌خواد بازی کنه دکمه رو بزنه:\n\nمیز: (۰/۲)",
            chat_id, call.message.message_id,
            reply_markup=bs_kb_lobby(table_id),
        )
    elif action == "poker":
        ...
        existing = load_poker_game(chat_id)
        if existing and existing.phase != "done":
            bot.answer_callback_query(call.id, "یه بازی پوکر همین الان تو این گروه در جریانه.", show_alert=True)
            return
        game = PokerGame(chat_id=chat_id)
        save_poker_game(game)
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"🃏 میز پوکر باز شد! (تگزاس هولدم، ورودی هر نفر {POKER_ANTE} شیهه‌پوینت)\n"
            f"بین {POKER_MIN_PLAYERS} تا {POKER_MAX_PLAYERS} نفر می‌تونن بازی کنن.\n\n"
            f"میز (۰/{POKER_MAX_PLAYERS}):",
            chat_id, call.message.message_id,
            reply_markup=poker_kb_lobby(chat_id, can_start=False),
        )
    elif action == "blackjack":
        chat_id = call.message.chat.id
        existing = load_blackjack_game(chat_id)
        if existing and existing.phase != "done":
            bot.answer_callback_query(call.id, "یه بازی بلک‌جک همین الان تو این گروه در جریانه.", show_alert=True)
            return
        game = BlackjackGame(chat_id=chat_id)
        game.lobby_message_id = call.message.message_id
        save_blackjack_game(game)
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"🂡 میز بلک‌جک باز شد! (شرط پیش‌فرض: {BJ_BET} شیهه‌پوینت)\n"
            f"می‌خوای شرط دلخواه بذاری؟ رو همین پیام ریپلای کن و فقط عددشو بفرست، بعد «ورود به میز» رو بزن.\n"
            f"تا {BJ_MAX_PLAYERS} نفر می‌تونن بازی کنن، هرکی جدا در برابر دیلره.\n\n"
            f"میز (۰/{BJ_MAX_PLAYERS}):",
            chat_id, call.message.message_id,
            reply_markup=bj_kb_lobby(chat_id, can_start=False),
        )
    else:
        bot.answer_callback_query(call.id)
...
# =========================================================
#              🎰 گمبل (شرط‌بندی با شیهه‌پوینت) 🎰
# =========================================================
# ایزوله از بقیه‌ی بازی‌ها: فقط از adjust_shihe() موجود استفاده می‌کنه
# (که خودش اتمیکه)، یه prefix جدا ("gamble:") تو دیسپچر مرکزی داره،
# مبلغ دلخواه از همون handle_digit_reply مرکزی گرفته می‌شه، و اسلات
# هم از همون دیسپچر مرکزیِ دایس (_dino_dart_handler) — نه یه
# message_handler جدا — که با رفتار first-match-breaks کتابخونه
# تداخل نکنه. مالکیت منو هم از قبل تو handle_callback چک می‌شه، پس
# لازم نیست خودمون chat_id/user_id رو تو callback_data تکرار کنیم.
#
# نکته‌ی همزمانی: چون ربات threaded=True (پیش‌فرض pyTelegramBotAPI)
# اجراست، دو تا کلیک/ریپلای هم‌زمان می‌تونن تو دو ترد جدا پردازش بشن.
# برای همین هر جا شرط پول جابه‌جا می‌شه، خوندن-و-تغییر state رو زیر
# قفل می‌بریم (streak_lock / slot_lock / mines_lock) تا دابل‌تپ یا
# ریپلای تکراری باعث دابل-پرداخت یا گم‌شدن شرط نشه.

GAMBLE_MIN_BET = 20
GAMBLE_MAX_BET = 3000
GAMBLE_PRESET_BETS = [20, 50, 100, 250, 500, 1000, 3000]

_streak_pending = {}          # key: (chat_id, user_id) -> {"game","bet","mult","extra","card"}  (شیر‌خط/تاس/بیشتر‌کمتر پیوسته)
_gamble_custom_pending = {}   # key: (chat_id, user_id) -> {"game": str, "extra": str|None, "message_id": int}
_slot_spin_pending = {}       # key: (chat_id, user_id) -> {"bet": int, "message_id": int|None}
_mines_games = {}             # key: (chat_id, user_id) -> {"bet", "mines": set, "revealed": set, "num_mines"}
...
def _validate_gamble_bet(bet_str):
    """اعتبارسنجی مبلغ شرط. خروجی: (ok, مقدار_یا_متن_خطا)"""
    try:
        bet = int(bet_str)
    except ValueError:
        return False, "مبلغ شرط باید عدد باشه."
    if bet < GAMBLE_MIN_BET or bet > GAMBLE_MAX_BET:
        return False, f"شرط باید بین {GAMBLE_MIN_BET} تا {GAMBLE_MAX_BET} شیهه‌پوینت باشه."
    return True, bet


def _gamble_back_kb():
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🔙 گمبل", callback_data="gamble:back"))
    return kb


def _stuck_game_kb(kind):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("❌ لغو بازی نیمه‌کاره و پس‌گرفتن پول", callback_data=f"gamble:cancelstuck:{kind}"))
    return kb


def cancel_stuck_streak(chat_id, user_id):
    """برای شیر‌خط/بیشتر‌کمتر/تاس — وقتی حالت قبلی گم شده و کاربر گیر کرده."""
    with streak_lock:
        state = _streak_pending.pop((chat_id, user_id), None)
    if not state:
        return False, "بازی نیمه‌کاره‌ای برات نمونده."
    bet = state["bet"]
    _, new_balance = adjust_shihe(user_id, bet)
    return True, f"❌ بازی لغو شد و {bet} شیهه‌پوینت برگشت.\n🌟 موجودی: {new_balance}"


def cancel_stuck_mines(chat_id, user_id):
    with mines_lock:
        state = _mines_games.pop((chat_id, user_id), None)
    if not state:
        return False, "بازی نیمه‌کاره‌ای برات نمونده."
    bet = state["bet"]
    _, new_balance = adjust_shihe(user_id, bet)
...
def cancel_stuck_slot(chat_id, user_id):
    with slot_lock:
        state = _slot_spin_pending.pop((chat_id, user_id), None)
    if not state:
        return False, "بازی نیمه‌کاره‌ای برات نمونده."
    bet = state["bet"]
    _, new_balance = adjust_shihe(user_id, bet)
...
# ---------- 🎛️ منوهای شیشه‌ای گمبل ----------

def gamble_kb_main_menu():
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🎰 اسلات", callback_data="gamble:pick:slot"))
    kb.add(types.InlineKeyboardButton("🪙 شیر یا خط", callback_data="gamble:pick:coin"))
    kb.add(types.InlineKeyboardButton("🃏 بیشتر یا کمتر", callback_data="gamble:pick:hilo"))
    kb.add(types.InlineKeyboardButton("🎲 تاس", callback_data="gamble:pick:dice"))
    kb.add(types.InlineKeyboardButton("💣 مین‌یاب", callback_data="gamble:pick:mines"))
    kb.add(types.InlineKeyboardButton("🔙 برگشت به منو", callback_data="menu:main"))
    return kb


def gamble_kb_coin_side():
    kb = types.InlineKeyboardMarkup()
    kb.add(
        types.InlineKeyboardButton("🦁 شیر", callback_data="gamble:coinside:شیر"),
        types.InlineKeyboardButton("〽️ خط", callback_data="gamble:coinside:خط"),
    )
    kb.add(types.InlineKeyboardButton("🔙 برگشت", callback_data="gamble:back"))
    return kb


def gamble_kb_dice_type():
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🔀 زوج (جمع دوتا تاس زوج باشه)", callback_data="gamble:dicetype:even"))
    kb.add(types.InlineKeyboardButton("🔀 فرد (جمع دوتا تاس فرد باشه)", callback_data="gamble:dicetype:odd"))
    kb.add(types.InlineKeyboardButton("🎯 عدد دقیق مجموع", callback_data="gamble:dicetype:exact"))
    kb.add(types.InlineKeyboardButton("🔙 برگشت", callback_data="gamble:back"))
    return kb


def gamble_kb_dice_numbers():
    kb = types.InlineKeyboardMarkup(row_width=4)
    buttons = [types.InlineKeyboardButton(str(n), callback_data=f"gamble:dicenum:{n}") for n in range(2, 13)]
    kb.add(*buttons)
    kb.add(types.InlineKeyboardButton("🔙 برگشت", callback_data="gamble:pick:dice"))
    return kb


def gamble_kb_mines_count():
    kb = types.InlineKeyboardMarkup()
    for m in MINES_OPTIONS:
        kb.add(types.InlineKeyboardButton(f"💣 {m} مین (از {MINES_TOTAL_CELLS} خونه)", callback_data=f"gamble:minecount:{m}"))
    kb.add(types.InlineKeyboardButton("🔙 برگشت", callback_data="gamble:back"))
    return kb


def gamble_kb_bet_amounts(game, extra=None):
    kb = types.InlineKeyboardMarkup(row_width=3)
    buttons = []
    for amt in GAMBLE_PRESET_BETS:
        if extra is not None:
            data = f"gamble:bet{game}:{extra}:{amt}"
        else:
            data = f"gamble:bet{game}:{amt}"
        buttons.append(types.InlineKeyboardButton(str(amt), callback_data=data))
    kb.add(*buttons)
    custom_data = f"gamble:custom{game}:{extra}" if extra is not None else f"gamble:custom{game}"
    kb.add(types.InlineKeyboardButton("✏️ مبلغ دلخواه", callback_data=custom_data))
    kb.add(types.InlineKeyboardButton("🔙 برگشت", callback_data="gamble:back"))
    return kb


def gamble_kb_higher_lower(cash_amount=None):
    kb = types.InlineKeyboardMarkup()
    kb.row(
        types.InlineKeyboardButton("🔼 بیشتر", callback_data="gamble:hl:higher"),
        types.InlineKeyboardButton("🔽 کمتر", callback_data="gamble:hl:lower"),
    )
    if cash_amount is not None:
        kb.add(types.InlineKeyboardButton(f"🏳️ برداشت ({cash_amount} شیهه‌پوینت)", callback_data="gamble:hlcash"))
    return kb


def gamble_kb_continue_or_cash(cont_action, cash_action, cash_amount):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🔁 دوباره", callback_data=cont_action))
    kb.add(types.InlineKeyboardButton(f"🏳️ برداشت ({cash_amount} شیهه‌پوینت)", callback_data=cash_action))
    return kb


@bot.message_handler(commands=["گمبل", "gamble"])
@group_only
def cmd_gamble_menu(message):
    sent = bot.reply_to(message, "🎰 گمبل — کدوم بازی؟", reply_markup=gamble_kb_main_menu())
    set_menu_owner(sent.chat.id, sent.message_id, message.from_user.id)


@bot.message_handler(commands=["لغو_مین", "cancel_mines"])
@group_only
def cmd_cancel_mines(message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    with mines_lock:
        state = _mines_games.get((chat_id, user_id))
    if state is None:
        bot.reply_to(message, "بازی مین‌یاب فعالی نداری.")
        return
    sent = bot.reply_to(
        message,
        f"⚠️ مطمئنی می‌خوای لغو کنی؟ شرط {state['bet']} شیهه‌پوینتیت از دست می‌ره.",
        reply_markup=mines_kb_confirm_cancel(state["bet"]),
    )
    set_menu_owner(sent.chat.id, sent.message_id, user_id)


# ---------- 🎰 اسلات (اسلات واقعیِ خود تلگرام) ----------
# به‌جای شبیه‌سازی رول‌ها تو خود ربات، از دایس/اسلات رسمی تلگرام استفاده
# می‌کنیم: بازیکن ایموجی 🎰 رو ریپلای می‌کنه، تلگرام انیمیشن واقعی رو
# نشون می‌ده، و ما نتیجه رو از همون message.dice.value واقعی می‌خونیم.

SLOT_TG_SYMBOLS = {0: "BAR", 1: "🍇", 2: "🍋", 3: "7️⃣"}
SLOT_TG_PAIR_MULT = 1.1
SLOT_TG_TRIPLE_MULT = 4
SLOT_TG_JACKPOT_MULT = 6


def decode_slot_value(value):
    """طبق مستندات تلگرام: value بین ۱ تا ۶۴، هر رول یه رقم بیس-۴ از (value-1)ه."""
    idx = value - 1
    d0 = idx % 4
    d1 = (idx // 4) % 4
    d2 = (idx // 16) % 4
...
def resolve_slot_value(user_id, bet, value):
    """bet از قبل کم شده؛ این فقط نتیجه رو حساب و جایزه رو (اگه بردی) واریز می‌کنه."""
    d0, d1, d2 = decode_slot_value(value)
    reel_text = " | ".join(SLOT_TG_SYMBOLS[d] for d in (d0, d1, d2))

    if d0 == d1 == d2:
        if d0 == 3:
            mult, tag = SLOT_TG_JACKPOT_MULT, "🎉 جکپات ۷۷۷ شد!"
        else:
            mult, tag = SLOT_TG_TRIPLE_MULT, "🎉 سه‌تایی شد!"
        winnings = int(bet * mult)
        _, new_balance = adjust_shihe(user_id, winnings)
        text = f"🎰 {reel_text}\n{tag} بردی: {winnings} شیهه‌پوینت (x{mult})\n🌟 موجودی: {new_balance}"
    elif d0 == d1 or d1 == d2 or d0 == d2:
        winnings = int(bet * SLOT_TG_PAIR_MULT)
        _, new_balance = adjust_shihe(user_id, winnings)
        text = f"🎰 {reel_text}\n✨ دوتایی! بردی: {winnings} شیهه‌پوینت (x{SLOT_TG_PAIR_MULT})\n🌟 موجودی: {new_balance}"
    else:
        new_balance = (get_user_row(user_id) or {}).get("shihe_points", 0)
        text = f"🎰 {reel_text}\n😢 باختی. {bet} شیهه‌پوینت رفت.\n🌟 موجودی: {new_balance}"

    return text


def start_slot_spin(chat_id, user_id, bet):
    """خروجی: (ok, text, keyboard|None). جای بازی رو قبل از کم‌کردن پول رزرو می‌کنیم که
    دو تا کلیک هم‌زمان هر دو نتونن از چک رد بشن و باعث گم‌شدن یه شرط نشن."""
    with slot_lock:
        if (chat_id, user_id) in _slot_spin_pending:
            return False, "یه اسلات برات باز مونده، اول اونو بچرخون.", _stuck_game_kb("slot")
        _slot_spin_pending[(chat_id, user_id)] = {"bet": bet, "message_id": None}

    taken, remaining = adjust_shihe(user_id, -bet)
    if not taken:
        with slot_lock:
            _slot_spin_pending.pop((chat_id, user_id), None)
        return False, f"شیهه‌پوینت کافی نداری! تو فقط {remaining} تا داری.", None

    text = "🎰 رو همین پیام ریپلای کن و ایموجی 🎰 بفرست تا بچرخونیم!"
    return True, text, None


def attach_slot_message_id(chat_id, user_id, message_id):
    with slot_lock:
        pending = _slot_spin_pending.get((chat_id, user_id))
        if pending is not None:
            pending["message_id"] = message_id


def handle_slot_spin_reply(message):
    """از دیسپچر مرکزی دایس (_dino_dart_handler) صدا زده می‌شه، نه مستقیم."""
    ...
    with slot_lock:
        pending = _slot_spin_pending.get((chat_id, user_id))
        if pending is None or pending["message_id"] != reply_id:
            return
        del _slot_spin_pending[(chat_id, user_id)]
        bet = pending["bet"]

    text = resolve_slot_value(user_id, bet, message.dice.value)
    bot.reply_to(message, text)
...
STREAK_MAX_MULTIPLIER = 50  # سقف مشترک برای همه‌ی بازی‌های پیوسته (سکه/تاس/کمتر‌بیشتر) — بعدش برداشت خودکار


def _play_coin_round(side):
    result = random.choice(["شیر", "خط"])
    return result == side, result


def start_coin_streak(chat_id, user_id, side, bet):
    with streak_lock:
        if (chat_id, user_id) in _streak_pending:
            return False, "یه بازی برات باز مونده، اول اونو تموم کن.", _stuck_game_kb("streak")
        _streak_pending[(chat_id, user_id)] = None

    taken, remaining = adjust_shihe(user_id, -bet)
    if not taken:
        with streak_lock:
            _streak_pending.pop((chat_id, user_id), None)
        return False, f"شیهه‌پوینت کافی نداری! تو فقط {remaining} تا داری.", None

    with streak_lock:
        _streak_pending[(chat_id, user_id)] = {"game": "coin", "bet": bet, "mult": 1.0, "extra": side}
    return continue_coin_streak(chat_id, user_id)


def continue_coin_streak(chat_id, user_id):
    with streak_lock:
        state = _streak_pending.get((chat_id, user_id))
        if state is None or state["game"] != "coin":
            return False, "این بازی منقضی شده.", None

        won, result = _play_coin_round(state["extra"])
        if not won:
            _streak_pending.pop((chat_id, user_id), None)
            bet = state["bet"]
        else:
            state["mult"] *= COINFLIP_WIN_MULTIPLIER
            if state["mult"] >= STREAK_MAX_MULTIPLIER:
                payout = int(state["bet"] * state["mult"])
                _streak_pending.pop((chat_id, user_id), None)
                _, new_balance = adjust_shihe(user_id, payout)
                text = (
                    f"🪙 سکه: {result}!\n🎉 بردی! به سقف ضریب رسیدی، خودکار برداشت شد: "
                    f"{payout} شیهه‌پوینت (x{state['mult']:.2f})\n🌟 موجودی: {new_balance}"
                )
                return True, text, _gamble_back_kb()
            _streak_pending[(chat_id, user_id)] = state
            payout = int(state["bet"] * state["mult"])
            mult_display = state["mult"]
    ...
    text = f"🪙 سکه: {result}!\n🎉 بردی! تا الان: {payout} شیهه‌پوینت (x{mult_display:.2f})\nادامه بدی یا برداشت کنی؟"
    return True, text, gamble_kb_continue_or_cash("gamble:coincont", "gamble:coincash", payout)


def cash_out_coin(chat_id, user_id):
    with streak_lock:
        state = _streak_pending.get((chat_id, user_id))
        if state is None or state["game"] != "coin":
            return False, "این بازی منقضی شده.", None
        del _streak_pending[(chat_id, user_id)]

    winnings = int(state["bet"] * state["mult"])
    _, new_balance = adjust_shihe(user_id, winnings)
    text = f"🏳️ برداشت کردی! بردی: {winnings} شیهه‌پوینت (x{state['mult']:.2f})\n🌟 موجودی: {new_balance}"
    return True, text, _gamble_back_kb()


# ---------- 🃏 بیشتر یا کمتر (پیوسته) ----------

HILO_WIN_MULTIPLIER = 1.8


def start_hilo(chat_id, user_id, bet):
    with streak_lock:
        if (chat_id, user_id) in _streak_pending:
            return False, "یه بازی برات باز مونده، اول اونو تموم کن.", _stuck_game_kb("streak")
        _streak_pending[(chat_id, user_id)] = None
    ...
    with streak_lock:
        _streak_pending[(chat_id, user_id)] = {"game": "hilo", "bet": bet, "mult": 1.0, "card": card}
    text = f"🃏 کارت اول: {card}\nحدس بزن کارت بعدی بیشتره یا کمتره:"
    return True, text, gamble_kb_higher_lower()


def resolve_hilo_guess(chat_id, user_id, guess):
    """خروجی: (ok, text, keyboard)"""
    with streak_lock:
        state = _streak_pending.get((chat_id, user_id))
        if state is None or state["game"] != "hilo":
            return False, "این شرط منقضی شده یا قبلاً جواب داده شده.", None

        first_card = state["card"]
        second_card = random.randint(1, 13)

        if second_card == first_card:
            cash_amount = int(state["bet"] * state["mult"]) if state["mult"] > 1 else None
            return "tie", (first_card, second_card, cash_amount), None

        won = (guess == "higher" and second_card > first_card) or (guess == "lower" and second_card < first_card)
        if won:
            state["mult"] *= HILO_WIN_MULTIPLIER
            state["card"] = second_card
            if state["mult"] >= STREAK_MAX_MULTIPLIER:
                payout = int(state["bet"] * state["mult"])
                _streak_pending.pop((chat_id, user_id), None)
                _, new_balance = adjust_shihe(user_id, payout)
                text = (
                    f"🃏 کارت اول: {first_card} | کارت دوم: {second_card}\n"
                    f"🎉 درست گفتی! به سقف ضریب رسیدی، خودکار برداشت شد: "
                    f"{payout} شیهه‌پوینت (x{state['mult']:.2f})\n🌟 موجودی: {new_balance}"
                )
                return True, text, _gamble_back_kb()
            _streak_pending[(chat_id, user_id)] = state
    ...
    if not won:
        new_balance = (get_user_row(user_id) or {}).get("shihe_points", 0)
        text = f"🃏 کارت اول: {first_card} | کارت دوم: {second_card}\n😢 باختی. {bet} شیهه‌پوینت رفت.\n🌟 موجودی: {new_balance}"
        return True, text, _gamble_back_kb()

    text = (
        f"🃏 کارت اول: {first_card} | کارت دوم: {second_card}\n"
        f"🎉 درست گفتی! تا الان: {payout} شیهه‌پوینت (x{mult_display:.2f})\n"
        f"کارت جدید: {second_card} — حدس بزن کارت بعدی بیشتره یا کمتره:"
    )
    return True, text, gamble_kb_higher_lower(payout)


def cash_out_hilo(chat_id, user_id):
    with streak_lock:
        state = _streak_pending.get((chat_id, user_id))
        if state is None or state["game"] != "hilo":
            return False, "این بازی منقضی شده.", None
...
DICE_EVEN_ODD_MULTIPLIER = 1.9
DICE_EXACT_MULTIPLIERS = {2: 30, 3: 16, 4: 10, 5: 8, 6: 6, 7: 5, 8: 6, 9: 8, 10: 10, 11: 16, 12: 30}


def _play_dice_round(dice_type):
    d1, d2 = random.randint(1, 6), random.randint(1, 6)
    total = d1 + d2
    line = f"🎲 {d1} + {d2} = {total}"
    if dice_type == "even":
        return total % 2 == 0, DICE_EVEN_ODD_MULTIPLIER, "زوج", line
    if dice_type == "odd":
        return total % 2 == 1, DICE_EVEN_ODD_MULTIPLIER, "فرد", line
    target = int(dice_type.replace("exact", ""))
    return total == target, DICE_EXACT_MULTIPLIERS.get(target, 8), f"عدد {target}", line


def start_dice_streak(chat_id, user_id, dice_type, bet):
    with streak_lock:
        if (chat_id, user_id) in _streak_pending:
            return False, "یه بازی برات باز مونده، اول اونو تموم کن.", _stuck_game_kb("streak")
        _streak_pending[(chat_id, user_id)] = None
    ...
    with streak_lock:
        _streak_pending[(chat_id, user_id)] = {"game": "dice", "bet": bet, "mult": 1.0, "extra": dice_type}
    return continue_dice_streak(chat_id, user_id)


def continue_dice_streak(chat_id, user_id):
    with streak_lock:
        state = _streak_pending.get((chat_id, user_id))
        if state is None or state["game"] != "dice":
            return False, "این بازی منقضی شده.", None

        won, round_mult, label, line = _play_dice_round(state["extra"])
        if not won:
            _streak_pending.pop((chat_id, user_id), None)
            bet = state["bet"]
        else:
            state["mult"] *= round_mult
            if state["mult"] >= STREAK_MAX_MULTIPLIER:
                payout = int(state["bet"] * state["mult"])
                _streak_pending.pop((chat_id, user_id), None)
                _, new_balance = adjust_shihe(user_id, payout)
                text = (
                    f"{line}\n🎉 شرطت ({label}) درست بود! به سقف ضریب رسیدی، خودکار برداشت شد: "
                    f"{payout} شیهه‌پوینت (x{state['mult']:.2f})\n🌟 موجودی: {new_balance}"
                )
                return True, text, _gamble_back_kb()
            _streak_pending[(chat_id, user_id)] = state
    ...
    if not won:
        new_balance = (get_user_row(user_id) or {}).get("shihe_points", 0)
        text = f"{line}\n😢 شرطت ({label}) جواب نداد. {bet} شیهه‌پوینت رفت.\n🌟 موجودی: {new_balance}"
        return True, text, _gamble_back_kb()

    text = f"{line}\n🎉 شرطت ({label}) درست بود! تا الان: {payout} شیهه‌پوینت (x{mult_display:.2f})\nادامه بدی یا برداشت کنی؟"
    return True, text, gamble_kb_continue_or_cash("gamble:dicecont", "gamble:dicecash", payout)


def cash_out_dice(chat_id, user_id):
    with streak_lock:
        state = _streak_pending.get((chat_id, user_id))
        if state is None or state["game"] != "dice":
            return False, "این بازی منقضی شده.", None
...
def mines_multiplier(total_cells, num_mines, revealed_count):
    mult = 1.0
    for i in range(revealed_count):
        remaining = total_cells - i
        safe_remaining = remaining - num_mines
        if safe_remaining <= 0:
            break
...
def mines_kb(chat_id, user_id):
    state = _mines_games[(chat_id, user_id)]
    kb = types.InlineKeyboardMarkup(row_width=5)
    row = []
    for idx in range(MINES_TOTAL_CELLS):
        label = "💎" if idx in state["revealed"] else "❔"
        row.append(types.InlineKeyboardButton(label, callback_data=f"gamble:mine:{idx}"))
        if len(row) == 5:
            kb.row(*row)
            row = []

    if state["revealed"]:
        mult = mines_multiplier(MINES_TOTAL_CELLS, state["num_mines"], len(state["revealed"]))
        payout = int(state["bet"] * mult)
        cash_label = f"🏳️ برداشت ({payout} شیهه‌پوینت, x{mult:.2f})"
    else:
        cash_label = "🏳️ برداشت"
    kb.row(types.InlineKeyboardButton(cash_label, callback_data="gamble:minecash"))
    kb.row(types.InlineKeyboardButton("🔙 لغو بازی", callback_data="gamble:mineback"))
    return kb


def mines_kb_confirm_cancel(bet):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("بله، لغو کن (شرطم بسوزه)", callback_data="gamble:mineconfirmyes"))
    kb.add(types.InlineKeyboardButton("نه، برگرد به بازی", callback_data="gamble:mineconfirmno"))
    return kb


def mines_final_kb(state, hit_idx=None):
    kb = types.InlineKeyboardMarkup(row_width=5)
    row = []
    for idx in range(MINES_TOTAL_CELLS):
        if idx == hit_idx:
            label = "💥"
        elif idx in state["mines"]:
            label = "💣"
        elif idx in state["revealed"]:
            label = "💎"
        else:
            label = "▫️"
        row.append(types.InlineKeyboardButton(label, callback_data="gamble:noop"))
        if len(row) == 5:
            kb.row(*row)
            row = []
    kb.row(types.InlineKeyboardButton("🔙 گمبل", callback_data="gamble:back"))
    return kb


def start_mines(chat_id, user_id, bet, num_mines):
    with mines_lock:
        if (chat_id, user_id) in _mines_games:
            return False, "یه بازی مین‌یاب برات باز مونده، اول اونو تموم کن.", _stuck_game_kb("mines")
        _mines_games[(chat_id, user_id)] = None

    taken, remaining = adjust_shihe(user_id, -bet)
    if not taken:
        with mines_lock:
            del _mines_games[(chat_id, user_id)]
        return False, f"شیهه‌پوینت کافی نداری! تو فقط {remaining} تا داری.", None

    mine_positions = set(random.sample(range(MINES_TOTAL_CELLS), num_mines))
    with mines_lock:
        _mines_games[(chat_id, user_id)] = {
            "bet": bet, "mines": mine_positions, "revealed": set(), "num_mines": num_mines,
        }
    text = (
        f"💣 مین‌یاب — {num_mines} مین رو {MINES_TOTAL_CELLS} خونه.\n"
        f"هر خونه که باز کنی ضریب بردت بالاتر می‌ره. هر وقت خواستی «برداشت» رو بزن."
    )
    return True, text, mines_kb(chat_id, user_id)


# ---------- 🎛️ دیسپچر گمبل ----------

def handle_gamble_callback(call):
    data = call.data
    parts = data.split(":")
    action = parts[1]
    user_id = call.from_user.id
    ...
    if action == "cancelstuck":
        kind = parts[2]
        if kind == "mines":
            ok, text = cancel_stuck_mines(chat_id, user_id)
        elif kind == "slot":
            ok, text = cancel_stuck_slot(chat_id, user_id)
        else:
            ok, text = cancel_stuck_streak(chat_id, user_id)
        bot.answer_callback_query(call.id)
        bot.edit_message_text(text, chat_id, call.message.message_id, reply_markup=_gamble_back_kb())
        return

    if action == "back":
        bot.answer_callback_query(call.id)
        bot.edit_message_text("🎰 گمبل — کدوم بازی؟", chat_id, call.message.message_id, reply_markup=gamble_kb_main_menu())
        return
    ...
    if action == "pick":
        ...
        bot.answer_callback_query(call.id)
        if game == "coin":
            bot.edit_message_text("🪙 شیر یا خط؟", chat_id, call.message.message_id, reply_markup=gamble_kb_coin_side())
        elif game == "dice":
            bot.edit_message_text("🎲 تاس — رو چی شرط می‌بندی؟", chat_id, call.message.message_id, reply_markup=gamble_kb_dice_type())
        elif game == "mines":
            bot.edit_message_text("💣 مین‌یاب — چند تا مین باشه؟", chat_id, call.message.message_id, reply_markup=gamble_kb_mines_count())
        else:
            label = "🎰 اسلات" if game == "slot" else "🃏 بیشتر یا کمتر"
            bot.edit_message_text(
                f"{label} — چقدر شرط می‌بندی؟",
                chat_id, call.message.message_id,
                reply_markup=gamble_kb_bet_amounts(game),
            )
        return
    ...
    if action == "coinside":
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"🪙 شیر یا خط ({side}) — چقدر شرط می‌بندی؟",
            chat_id, call.message.message_id,
            reply_markup=gamble_kb_bet_amounts("coin", side),
        )
        return
    ...
    if action == "dicetype":
        ...
        bot.answer_callback_query(call.id)
        if subtype == "exact":
            bot.edit_message_text("🎯 مجموع دوتا تاس چند باشه؟", chat_id, call.message.message_id, reply_markup=gamble_kb_dice_numbers())
        else:
            label = "زوج" if subtype == "even" else "فرد"
            bot.edit_message_text(
                f"🎲 تاس ({label}) — چقدر شرط می‌بندی؟",
                chat_id, call.message.message_id,
                reply_markup=gamble_kb_bet_amounts("dice", subtype),
            )
        return
    ...
    if action == "dicenum":
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"🎯 عدد {n} — چقدر شرط می‌بندی؟",
            chat_id, call.message.message_id,
            reply_markup=gamble_kb_bet_amounts("dice", f"exact{n}"),
        )
        return
    ...
    if action == "minecount":
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"💣 مین‌یاب ({num_mines} مین) — چقدر شرط می‌بندی؟",
            chat_id, call.message.message_id,
            reply_markup=gamble_kb_bet_amounts("mines", num_mines),
        )
        return

    if action.startswith("bet"):
        game = action[3:]  # slot | coin | hilo | dice | mines

        if game == "slot":
            bet = int(parts[2])
            ok, text, kb = start_slot_spin(chat_id, user_id, bet)
            if not ok:
                if kb is not None:
                    bot.answer_callback_query(call.id)
            ...
            attach_slot_message_id(chat_id, user_id, call.message.message_id)
            return

        if game == "coin":
            side = parts[2]
            bet = int(parts[3])
            ok, text, keyboard = start_coin_streak(chat_id, user_id, side, bet)
        elif game == "dice":
            dice_type = parts[2]
            bet = int(parts[3])
            ok, text, keyboard = start_dice_streak(chat_id, user_id, dice_type, bet)
        elif game == "mines":
            num_mines = int(parts[2])
            bet = int(parts[3])
            ok, text, keyboard = start_mines(chat_id, user_id, bet, num_mines)
        else:  # hilo
            bet = int(parts[2])
            ok, text, keyboard = start_hilo(chat_id, user_id, bet)
    ...
    if action.startswith("custom"):
        game = action[6:]  # slot | coin | hilo | dice | mines
        extra = parts[2] if len(parts) > 2 else None
        bot.answer_callback_query(call.id)
        prompt = f"رو همین پیام ریپلای کن و مبلغ شرطت رو بفرست (بین {GAMBLE_MIN_BET} تا {GAMBLE_MAX_BET} شیهه‌پوینت):"
        bot.edit_message_text(prompt, chat_id, call.message.message_id)
        _gamble_custom_pending[(chat_id, user_id)] = {
            "game": game, "extra": extra, "message_id": call.message.message_id,
        }
        return

    if action == "coincont":
        ok, text, keyboard = continue_coin_streak(chat_id, user_id)
        if not ok:
            bot.answer_callback_query(call.id, text, show_alert=True)
            return
        bot.answer_callback_query(call.id)
    ...
    if action == "coincash":
        ok, text, keyboard = cash_out_coin(chat_id, user_id)
        if not ok:
            bot.answer_callback_query(call.id, text, show_alert=True)
            return
        bot.answer_callback_query(call.id)
    ...
    if action == "dicecont":
        ok, text, keyboard = continue_dice_streak(chat_id, user_id)
        if not ok:
            bot.answer_callback_query(call.id, text, show_alert=True)
            return
        bot.answer_callback_query(call.id)
    ...
    if action == "dicecash":
        ok, text, keyboard = cash_out_dice(chat_id, user_id)
        if not ok:
            bot.answer_callback_query(call.id, text, show_alert=True)
            return
        bot.answer_callback_query(call.id)
    ...
    if action == "hlcash":
        ok, text, keyboard = cash_out_hilo(chat_id, user_id)
        if not ok:
            bot.answer_callback_query(call.id, text, show_alert=True)
            return
        bot.answer_callback_query(call.id)
    ...
    if action == "hl":
        guess = parts[2]
        result = resolve_hilo_guess(chat_id, user_id, guess)
        ok = result[0]
        if not ok:
            bot.answer_callback_query(call.id, result[1], show_alert=True)
            return
        if ok == "tie":
            first_card, second_card, cash_amount = result[1]
            bot.answer_callback_query(call.id, "🤝 مساوی شد، دوباره حدس بزن.")
            bot.edit_message_text(
                f"🃏 کارت اول: {first_card} | کارت دوم: {second_card} — مساوی، دوباره امتحان کن:\nحدس بزن کارت بعدی بیشتره یا کمتره:",
                chat_id, call.message.message_id, reply_markup=gamble_kb_higher_lower(cash_amount),
            )
            return
        text, keyboard = result[1], result[2]
        bot.answer_callback_query(call.id)
        bot.edit_message_text(text, chat_id, call.message.message_id, reply_markup=keyboard)
        return
    ...
    if action == "mineback":
        with mines_lock:
            state = _mines_games.get((chat_id, user_id))
        if state is None:
            bot.answer_callback_query(call.id, "بازی‌ای در جریان نیست.", show_alert=True)
            return
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"⚠️ مطمئنی می‌خوای لغو کنی؟ شرط {state['bet']} شیهه‌پوینتیت از دست می‌ره.",
    ...
    if action == "mineconfirmyes":
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"لغو شد. {state['bet']} شیهه‌پوینتی که شرط بسته بودی برنمی‌گرده.",
            chat_id, call.message.message_id,
            reply_markup=_gamble_back_kb(),
        )
        return

    if action == "mineconfirmno":
        with mines_lock:
            state = _mines_games.get((chat_id, user_id))
        if state is None:
            bot.answer_callback_query(call.id, "بازی‌ای در جریان نیست.", show_alert=True)
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"💣 مین‌یاب — {state['num_mines']} مین رو {MINES_TOTAL_CELLS} خونه.\nهر خونه که باز کنی ضریب بردت بالاتر می‌ره. هر وقت خواستی «برداشت» رو بزن.",
            chat_id, call.message.message_id,
            reply_markup=mines_kb(chat_id, user_id),
        )
        return
    ...
    if action == "mine":
        ...
        with mines_lock:
            ...
            if idx in state["revealed"]:
                bot.answer_callback_query(call.id, "این خونه رو قبلاً باز کردی.")
                return

            if idx in state["mines"]:
                del _mines_games[(chat_id, user_id)]
                bot.answer_callback_query(call.id, "💥 مین بود!", show_alert=True)
                bot.edit_message_text(
                    f"💥 ترکید! {state['bet']} شیهه‌پوینت رفت.",
                    chat_id, call.message.message_id,
                    reply_markup=mines_final_kb(state, hit_idx=idx),
                )
                return

            state["revealed"].add(idx)
            all_safe_found = len(state["revealed"]) == MINES_TOTAL_CELLS - state["num_mines"]
            if all_safe_found:
                mult = mines_multiplier(MINES_TOTAL_CELLS, state["num_mines"], len(state["revealed"]))
        ...
        if all_safe_found:
            _, new_balance = adjust_shihe(user_id, winnings)
            bot.answer_callback_query(call.id, "🎉 همه‌ی خونه‌های امن رو پیدا کردی!", show_alert=True)
            bot.edit_message_text(
                f"🎉 همه‌ی خونه‌های امن باز شد! بردی: {winnings} شیهه‌پوینت (x{mult:.2f})\n🌟 موجودی: {new_balance}",
                chat_id, call.message.message_id,
                reply_markup=_gamble_back_kb(),
            )
            return
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_reply_markup(chat_id, call.message.message_id, reply_markup=mines_kb(chat_id, user_id))
        return
    ...
    if action == "minecash":
        with mines_lock:
            ...
            if not state["revealed"]:
                bot.answer_callback_query(call.id, "هنوز هیچ خونه‌ای باز نکردی.", show_alert=True)
                return
            mult = mines_multiplier(MINES_TOTAL_CELLS, state["num_mines"], len(state["revealed"]))
            winnings = int(state["bet"] * mult)
            del _mines_games[(chat_id, user_id)]
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"🏳️ برداشت کردی! بردی: {winnings} شیهه‌پوینت (x{mult:.2f})\n🌟 موجودی: {new_balance}",
            chat_id, call.message.message_id,
            reply_markup=_gamble_back_kb(),
        )
        return
...
if __name__ == "__main__":
    ...
    init_hokm_tables()
    print("جدول بازی حکم آماده شد.")

    init_battleship_tables()
    print("جدول بازی نبرد دریایی آماده شد.")

    init_poker_tables()
    print("جدول بازی پوکر آماده شد.")

    init_blackjack_tables()
    print("جدول بازی بلک‌جک آماده شد.")
    ...
    salary_reminder_thread = threading.Thread(target=crime_gang.salary_reminder_loop, daemon=True)
    salary_reminder_thread.start()
    print("یادآوری حقوق گنگ فعال شد.")
    ...
    def _heartbeat_loop():
        # هر ۲ دقیقه یه خط تو لاگ چاپ می‌کنه. اگه یه‌بار دیگه ربات "بی‌صدا"
        # از کار افتاد (نه ارور، نه کرش)، از رو آخرین heartbeat تو لاگ رندر
        # می‌فهمیم دقیقاً کِی و کجا کل پروسه گیر کرده، به‌جای این‌که کورکورانه حدس بزنیم.
        while True:
            try:
                _last_alive[0] = time.monotonic()
                print(f"💓 heartbeat - ربات زنده‌ست - {datetime.now().strftime('%H:%M:%S')}", flush=True)
            except Exception:
                pass
            time.sleep(120)

    def _watchdog_loop():
        # اگه ۵ دقیقه از آخرین هارت‌بیت گذشت، یعنی حتی سبک‌ترین تردِ ممکن هم
        # نتونسته اجرا بشه — به‌جای امید بستن به اینکه یه‌جا خودش درست بشه،
        # پروسه رو می‌کشیم تا Render طبق تنظیمات ری‌استارتش کنه.
        while True:
            time.sleep(60)
            try:
                if time.monotonic() - _last_alive[0] > 300:
                    print("🚨 واچ‌داگ: ۵ دقیقه هیچ نشونه‌ای از زنده‌بودن نبود، پروسه رو می‌بندم تا ری‌استارت بشه", flush=True)
                    os._exit(1)
            except Exception:
                pass
    ...
    while True:
        try:
            ...
            if "409" in str(e) or "Conflict" in str(e):
                # 409 یعنی یه instance دیگه (معمولاً نسخه‌ی قبلیِ در حال خاموش
                # شدنِ رندر موقع دیپلوی) هنوز داره پولینگ می‌کنه. صبر بیشتر
                # می‌کنیم تا اون instance قدیمی کامل بمیره، بعد دوباره تلاش می‌کنیم.
                wait_seconds = 30
                print("⚠️ ارور 409: احتمالاً یه instance دیگه هنوز در حال polling با همین توکنه (معمولاً موقع دیپلوی رندر). صبر می‌کنیم تا بمیره...")
            else:
                print(f"⚠️ ربات با خطا متوقف شد: {e}")
            print(f"{wait_seconds} ثانیه صبر می‌کنیم و دوباره تلاش می‌کنیم...")
