...
def handle_version(message):
    ...
    bot.reply_to(message, "🔖 نسخه: v-2026-08-22-war-deadline-fix")
...
def send_war_announcement():
    ...
    with db_lock:
        ...
        try:
            conn.execute(
                "INSERT INTO dino_war_rounds (round_key, announced, announced_at) VALUES (?, 1, ?) "
                "ON CONFLICT(round_key) DO UPDATE SET announced=1, announced_at=excluded.announced_at",
                (round_key, iran_now().isoformat())
            )
        except Exception:
            pass
        conn.commit()
...
def force_send_war_announcement():
    ...
    # و زمان اعلام واقعی رو ذخیره می‌کنیم تا مهلت ۲.۵ ساعته از همین لحظه حساب بشه
    with db_lock:
        conn = get_conn()
        try:
            conn.execute(
