...
from hokm_engine import HokmGame, HokmError, Card, SUITS, SUIT_NAMES_FA, Phase
...
def handle_version(message):
    ...
    bot.reply_to(message, "🔖 نسخه: v-2026-08-30-add-hokm-game")
...
# این بخش فقط اتصال اون موتور به تلگرام و دیتابیسه، دقیقاً با همون
# db_lock / get_conn / PK_AUTOINCREMENT که بقیه‌ی بازی ازش استفاده می‌کنه.
...
def cmd_hokm(message):
    ...
    game = HokmGame(chat_id=chat_id)
    save_hokm_game(game)
    bot.send_message(
        chat_id,
        "🃏 بازی حکم شروع شد!\nمنتظر ۴ نفرم — هرکی می‌خواد بازی کنه دکمه رو بزنه:\n\nمیز: (۰/۴)",
...
def hokm_handle_join(call):
    ...
    if not hokm_can_pm(user.id):
        bot.answer_callback_query(
            call.id,
            "اول باید تو پیوی ربات /start بزنی، بعد وارد میز بشی (چون کارتاتو باید خصوصی بفرستم).",
            show_alert=True,
        )
        return
    ...
    bot.edit_message_text(
        f"🃏 بازی حکم شروع شد!\nمنتظر بقیه‌ایم...\n\nمیز ({len(game.players)}/۴): {names}",
        chat_id, call.message.message_id,
        reply_markup=hokm_kb_join() if len(game.players) < 4 else None,
    )
...
def hokm_start_new_hand(chat_id):
    ...
    try:
        ...
        bot.send_message(
            chat_id,
            f"⚠️ نتونستم به {game.player_names[hakem_id]} پیام خصوصی بدم. "
            "باید اول تو پیوی ربات /start بزنه، بعد بازی رو ادامه می‌دیم.",
        )
...
def hokm_handle_choose_suit(call, chat_id, suit):
    ...
    for pid in game.players:
        try:
            ...
            bot.send_message(chat_id, f"⚠️ نتونستم به {game.player_names[pid]} پیام بدم — باید /start بزنه.")
...
def hokm_send_turn_prompt(game):
    ...
    try:
        ...
        bot.send_message(
            game.chat_id,
            f"⚠️ نوبت {game.player_names[uid]}ه ولی نمی‌تونم پیوی بدم — باید /start بزنه تا بازی ادامه پیدا کنه.",
        )
...
def hokm_handle_play_card(call, chat_id, card_str):
    ...
    if result:
        ...
        team_label = "🔵 تیم ۱" if result["team"] == 0 else "🔴 تیم ۲"
        bot.send_message(
            chat_id,
    ...
    if game.phase == Phase.PLAYING:
        ...
        team_label = "🔵 تیم ۱" if winner_team == 0 else "🔴 تیم ۲"
        bot.send_message(
            chat_id,
