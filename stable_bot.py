            ...
            try:
                if _pg_conn.closed:
                    ...
                    if _pg_conn.get_transaction_status() != psycopg2.extensions.TRANSACTION_STATUS_IDLE:
                        _pg_conn.rollback()
                    # تست سلامت واقعی اتصال: Supabase/pgbouncer بعضی وقتا اتصالای
                    # بی‌کار رو بعد از چند دقیقه بی‌سروصدا سمت سرور می‌بنده، بدون
                    # این‌که پایتون خودش بفهمه (conn.closed هنوز False می‌مونه).
                    # یه کوئری سبک می‌زنیم تا مطمئن بشیم واقعاً زنده‌ست.
                    cur = _pg_conn.cursor()
