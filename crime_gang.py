...
LAUNDER_CAP_BASE = 250         # سقف تطهیر در ۲۴ ساعت، برای گنگ سطح ۱
...
def run_crime_job(worker: dict, gang: dict, with_ally: bool = False):
    ...
    if random.random() < success_chance:
        ...
        return {"status": "success", "message": f"✅ {worker['name']} تو «{crime['name']}»{tag} موفق شد! {reward} سکه‌ی کثیف به خزانه‌ی گنگ اضافه شد — با /تطهیر تمیزش کن."}
    ...
    if random.random() < 0.5:
        ...
        if random.random() < escape_chance:
            ...
            if worker.get("trait") == "coldblood":
                jail_hours = max(JAIL_HOURS_MIN, jail_hours // 2)
            jail_hours = max(JAIL_HOURS_MIN, round(jail_hours * (1 - _equip_bonus(worker, "gear", "jail_cut"))))
            if bribed:
                fine = round(fine * 0.4)
                jail_hours = max(JAIL_HOURS_MIN, jail_hours // 2)
...
def run_joint_crime_job(worker_a, gang_a, worker_b, gang_b, crime):
    ...
    if random.random() < success_chance:
        ...
        add_dirty_coins(gang_a["gang_id"], half_a)
        add_dirty_coins(gang_b["gang_id"], half_b)
        ...
        update_worker(worker_a["worker_id"], busy_until=(_now() + timedelta(hours=cooldown_h)).isoformat())
        update_worker(worker_b["worker_id"], busy_until=(_now() + timedelta(hours=cooldown_h)).isoformat())
        return {
            "status": "success",
            "message": (
                f"✅ {worker_a['name']} و {worker_b['name']} با هم «{crime['name']}» رو زدن!\n"
                f"💰 {half_a} سکه‌ی کثیف برای «{gang_a['name']}» و {half_b} سکه‌ی کثیف برای «{gang_b['name']}».\n"
