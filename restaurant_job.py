...
def _display_name(user) -> str:
    return (getattr(user, "first_name", None) or getattr(user, "username", None) or "بازیکن")


def _new_session(user_id, chat_id, name, message_id=None) -> dict:
    state = _get_state(user_id)
    return dict(
        chat_id=chat_id,
        ...
        oid=0,
        name=name,
        message_id=message_id,   # پیامی که همه‌ی سفارش‌ها تو همون ادیت می‌شن
        last_note="",            # نتیجه‌ی سفارش قبلی که بالای پیام نشون داده می‌شه
    )


def _put_message(chat_id, message_id, text, reply_markup=None):
    """اگه message_id هست همون پیام رو ادیت می‌کنه؛ اگه نبود یا ادیت نشد، پیام جدید می‌فرسته.
    message_id نهایی رو برمی‌گردونه."""
    if message_id:
        for attempt in range(2):
            try:
                ...
                if "not found" in msg or "can't be edited" in msg or "message_id_invalid" in msg:
                    break           # پیام پاک شده — پیام جدید می‌فرستیم
                time.sleep(1)       # مثلاً 429؛ یه بار دیگه امتحان می‌کنیم
    sent = bot.send_message(chat_id, text, reply_markup=reply_markup)
    return sent.message_id


def _order_text(name, recipe, picks, orders_left, seconds_left, note="") -> str:
    plate = "".join(picks) if picks else "هنوز خالیه"
    count = f" ({len(picks)})" if picks else ""
    head = f"🍽️ رستوران {name}\n"
    if note:
        head += f"{note}\n"
    return (
        f"{head}\n"
...
def _send_next_customer(user_id, chat_id):
    """سفارش بعدی رو تو همون پیامِ شیفت ادیت می‌کنه (پیام جدید نمی‌فرسته، مگه پیام اولیه نباشه)."""
    ...
    with _state_lock:
        ...
        mid = session.get("message_id")
        # تا وقتی پیام آپدیت نشده، سفارش «فعال» حساب نمی‌شه
        session.pop("recipe", None)

    try:
        mid = _put_message(
            chat_id, mid,
            _order_text(name, recipe, [], orders_left, limit, note),
            _order_kb(oid),
        )
    except Exception:
...
def _order_timeout(user_id, chat_id, oid):
    ...
    _set_note(user_id, f"⏰ «{taken['recipe']['name']}»: وقت تموم شد، مشتری رفت.")
...
def _end_shift(user_id, chat_id, timed_out=False):
    ...
    if timer:
        timer.cancel()
    lines = [f"🍽️ رستوران {session.get('name', '')}"]
    if session.get("last_note"):
        lines.append(session["last_note"])
    if "recipe" in session:
        lines.append(f"↩️ سفارش «{session['recipe']['name']}» نیمه‌کاره موند.")
    lines.append(f"\n🔚 شیفت تموم شد! {session['shift_earned']} سکه در آوردی.")
    try:
        _put_message(chat_id, session.get("message_id"), "\n".join(lines), _main_menu_kb(user_id))
    except Exception:
        pass


def _owner_of_message(chat_id, message_id):
    with _state_lock:
        for uid, s in _active_orders.items():
            if s.get("chat_id") == chat_id and s.get("message_id") == message_id:
                return uid, (s.get("name") or "یه بازیکن")
    return None


def _deny_click(call):
    owner = _owner_of_message(call.message.chat.id, call.message.message_id)
    if owner and owner[0] != call.from_user.id:
        bot.answer_callback_query(call.id, f"این سفارش مال «{owner[1]}» هست 🍽️", show_alert=True)
    else:
        bot.answer_callback_query(call.id, "این سفارش تموم شده.")
...
def _finish_order(call, oid):
    ...
    picks = taken["picks"]
    # ترتیب درست ولی برعکس هم قبوله
    if picks == expected or picks == expected[::-1]:
        state = _get_state(user_id)
        ...
        extra = f" (+{int(bonus*100)}٪ بونوس)" if bonus > 0 else ""
        _set_note(user_id, f"✅ «{recipe['name']}»: +{final_pay} سکه{extra} — استریک {new_streak}")
        bot.answer_callback_query(call.id, f"+{final_pay} سکه")
    else:
        _update_state(user_id, streak=0)
        _set_note(
            user_id,
            f"❌ «{recipe['name']}» اشتباه بود.\n"
            f"تو زدی: {''.join(picks) or '—'}\n"
            f"درست: {recipe['emojis']}",
        )
...
def _handle_order_callback(call):
    ...
    _safe_edit(
        ...
        _order_text(name, recipe, snapshot, orders_left, seconds_left, note),
    ...
    if data == "restaurant:open":
        with _state_lock:
            if user_id in _active_orders:
                ...
                return
            state = _get_state(user_id)
            _active_orders[user_id] = _new_session(user_id, chat_id, _display_name(call.from_user), call.message.message_id)
    ...
    def _cmd_open(message):
        ...
        with _state_lock:
            if user_id in _active_orders:
                ...
                return
            state = _get_state(user_id)
            _active_orders[user_id] = _new_session(user_id, chat_id, _display_name(message.from_user))
        _send_next_customer(user_id, chat_id)
